# 新增本地工程 Sync_DDR（2026-09-17）
[Sync_DDR](Sync_DDR/README_ZH.md) 是与下方四工程平级的独立 VHDL DDR 读写工程。用户要求由唯一新任务“同步 DDR｜设计、执行、复核与验证”统一完成设计、执行、复核和验证，原同步前端配对停止 DDR 工作。当前处于迁移后设计审查与执行入口修复阶段，读写功能尚未验证。
本节是新本地工程入口；下方四工程私有仓库发布记录保留，Sync_DDR 尚未包含在该次远程提交中。其他工程的设计/测试暂停状态不变。
