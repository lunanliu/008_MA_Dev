...
rtl=lines(root/'rtl/sources_a05.f');ips=lines(root/'ip/sources_a04.f')
...
lock=json.loads((root/'docs/jobs/OTA004_A05_source_lock.json').read_text(encoding='utf-8-sig'))
...
scan += [(root/'sim/tb/sync_ota_smoke_tb_a05.sv').resolve()]
...
if static_only:
 print(json.dumps(dict(status='OTA004_A05_STATIC_DEPENDENCIES_PASS',**dependencies),indent=2))
 sys.exit(0)
...
assert mods['sync_ota_top']=='rtl/control/sync_ota_top_a05.sv'
