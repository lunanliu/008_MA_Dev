...
spec=importlib.util.spec_from_file_location('r2',R/'tools/verify_ota004_a06r3_report.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
t=tkinter.Tcl();t.eval((R/'tools/vivado/audit_ota004_a06r3_gray.tcl').read_text())
...
with tempfile.TemporaryDirectory(prefix='ota_a06r3_realexports_') as tmp:
 out=Path(tmp);(out/'synth_applied_constraints.xdc').write_text(applied)
 t.call('ota_a06r3_gray_audit',out.as_posix())
