...
function Assert-OtaLaunchInput {
 ...
 if($StageValue -cnotin @('identity')){throw 'stage must be exactly identity'}
 $cap=600
 ...
 $expected=[IO.Path]::GetFullPath('D:/008_MA_Dev/Sync_OTA/tools/vivado/run_ota004_a06r2i1.tcl')
 $parent=[IO.Path]::GetFullPath('D:/008_MA_Dev/Sync_OTA/work/OTA004').TrimEnd([char]92,[char]47)
 if(-not [String]::Equals($entry,$expected,[StringComparison]::OrdinalIgnoreCase)){throw 'source is not the frozen A06R2I1 entry'}
 ...
 if([IO.Path]::GetFileName($candidate) -cnotmatch ('^'+$StageValue+'_a06r2i1(?:_[A-Za-z0-9-]+)?$')){throw 'attempt stage/name does not match frozen revision'}
 ...
 $manifest=Get-Content -LiteralPath 'D:/008_MA_Dev/Sync_OTA/docs/jobs/OTA004_A06R2I1_source_lock.json' -Raw | ConvertFrom-Json
 $entryRows=@($manifest.files | Where-Object {$_.path -ceq 'tools/vivado/run_ota004_a06r2i1.tcl'})
