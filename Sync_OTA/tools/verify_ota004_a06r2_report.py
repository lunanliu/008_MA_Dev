"""Fail-closed A06R2 report audit; callable parsers also support offline fixtures."""
...
def audit_gray(out):
 ...
 require((directory/'completed.txt').read_text().strip()=='OTA004_A06R2_GRAY_EXPORT_COMPLETE groups=24','Gray export incomplete')
...
def precheck(root,out):
 lock_path=root/'docs/jobs/OTA004_A06R2_source_lock.json'
 ...
 record=dict(status='A06R2_RECOVERY_PRECHECK_PASS',files=len(lock['files']),manifest_sha256=hashlib.sha256(lock_path.read_bytes()).hexdigest(),dcp_sha256=lock['dcp_sha256'],scope='report-only; existing DCP opened without source rebuild')
...
def main():
 ...
 require(out.parent==root/'work/OTA004' and out.name.startswith('report_a06r2'),'wrong recovery attempt')
 ...
 require((out/'timing_coverage/completed.txt').read_text().strip()=='OTA004_A06R2_BOUNDED_TIMING_EXPORT_COMPLETE queries=9 max_paths_total=116','bounded timing export incomplete')
