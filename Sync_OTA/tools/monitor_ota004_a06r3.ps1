...
function Assert-OtaLaunchInput {
 ...
 if($StageValue -cnotin @('report')){throw 'stage must be exactly report'}
 $cap=2700
 ...
 $expected=[IO.Path]::GetFullPath('D:/008_MA_Dev/Sync_OTA/tools/vivado/run_ota004_a06r3.tcl')
 $repairPrefix=[IO.Path]::GetFullPath('D:/008_MA_Dev/Sync_OTA/work/OTA004/a06r3_repairs').TrimEnd([char]92,[char]47)+[IO.Path]::DirectorySeparatorChar
 $initial=[String]::Equals($entry,$expected,[StringComparison]::OrdinalIgnoreCase)
 $repair=$entry.StartsWith($repairPrefix,[StringComparison]::OrdinalIgnoreCase) -and ($entry.Replace([char]92,[char]47) -cmatch '/a06r3_repairs/[A-Za-z0-9_-]+/tools/vivado/run_ota004_a06r3\.tcl$')
 if(-not ($initial -or $repair)){throw 'source must be initial A06R3 or a scoped execution-only revision bundle'}
 ...
 if([IO.Path]::GetFileName($candidate) -cnotmatch ('^'+$StageValue+'_a06r3(?:_[A-Za-z0-9-]+)?$')){throw 'attempt stage/name does not match frozen revision'}
 ...
 # Execution scripts may change within the authorized repair scope. Record
 # actual identities; scientific lock is separately enforced by the entry.
 $bundle=[IO.Path]::GetDirectoryName([IO.Path]::GetDirectoryName([IO.Path]::GetDirectoryName($entry)))
 $deps=@('tools/vivado/run_ota004_a06r3.tcl','tools/vivado/audit_ota004_a06r3_identity.tcl','tools/vivado/audit_ota004_a06r3_gray.tcl','tools/vivado/report_ota004_a06r3_timing.tcl','tools/verify_ota004_a06r3_report.py')
 $identities=@()
 foreach($rel in $deps){
  $dep=Join-Path $bundle $rel
  if(-not (Test-Path -LiteralPath $dep -PathType Leaf)){throw ('execution dependency missing: '+$dep)}
  $probe=$dep
  while($probe){
   $item=Get-Item -LiteralPath $probe -Force -ErrorAction Stop
   if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0){throw 'reparse execution dependency forbidden'}
   $next=[IO.Path]::GetDirectoryName($probe);if($next -eq $probe){break};$probe=$next
  }
  $identities+= [PSCustomObject]@{path=$dep;sha256=(Get-FileHash -LiteralPath $dep -Algorithm SHA256).Hash.ToLowerInvariant()}
 }
 [PSCustomObject]@{attempt=$candidate;source=$entry;stage=$StageValue;stage_cap_seconds=$cap;bundle=$bundle;execution_files=$identities;execution_revision=$repair}
...
$executionRecord=[ordered]@{recorded_utc=[DateTime]::UtcNow.ToString('o');source=$sourcePath;stage=$Stage;absolute_deadline_utc=$AbsoluteDeadlineUtc;hard_seconds=$HardSeconds;monitor=$PSCommandPath;monitor_sha256=(Get-FileHash -LiteralPath $PSCommandPath -Algorithm SHA256).Hash.ToLowerInvariant();execution_files=$validated.execution_files;execution_revision=$validated.execution_revision;scope='Execution identity recorded, not scientific acceptance'}
$executionRecord | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $attempt 'actual_execution_identity.json') -Encoding utf8
$archive=Join-Path $attempt 'executed_scripts';New-Item -ItemType Directory -Path $archive | Out-Null
foreach($row in $validated.execution_files){Copy-Item -LiteralPath $row.path -Destination (Join-Path $archive ([IO.Path]::GetFileName($row.path))) -ErrorAction Stop}
Copy-Item -LiteralPath $PSCommandPath -Destination (Join-Path $archive 'monitor_ota004_a06r3.ps1') -ErrorAction Stop
