...
module cfo_fft2048_core(
 ...
 localparam [1:0] LOAD=0,RUN=1,OUTPUT=2,ERROR_HOLD=3;
 logic [1:0] state;
 ...
 function automatic signed [25:0] clamp26(input logic signed [27:0] x);
  begin
   if(x>28'sd33554431)clamp26=26'sh1ffffff;
   else if(x< -28'sd33554432)clamp26=26'sh2000000;
   else clamp26=x[25:0];
  end
 endfunction
 function automatic clipped26(input logic signed [27:0] x);
  begin clipped26=(x>28'sd33554431 || x< -28'sd33554432);end
 endfunction
 always_comb begin
  ...
  if(!rst && !abort_sync)begin
   ...
   if(state==RUN)begin
    ...
    if(v[6])begin
     we0=1;we1=1;
     waddr0=tag[6].bank_a ? tag[6].row_b : tag[6].row_a;
     waddr1=tag[6].bank_a ? tag[6].row_a : tag[6].row_b;
     wdata0=tag[6].bank_a ? {next_bi,next_bq} : {next_ai,next_aq};
     wdata1=tag[6].bank_a ? {next_ai,next_aq} : {next_bi,next_bq};
    end
   end
  ...
  if(rst || abort_sync)begin
   ...
   case(state)
    LOAD:if(s_valid)begin
     ...
     if(input_error!=0)begin state<=ERROR_HOLD;m_error<=input_error;m_index<=0;m_last<=1;m_i<=0;m_q<=0;m_saturations<=0;end
     else if(input_index==11'd2047)begin
      state<=RUN;stage_index<=0;half_span<=1;tw_stride<=1024;offset_in_group<=0;group_base<=0;tw_index<=0;issue_index<=0;issued_all<=0;v<=0;
     end else input_index<=input_index+11'd1;
    end
    RUN:begin
     v<={v[5:0],!issued_all};
     for(t=1;t<7;t=t+1)if(v[t-1])tag[t]<=tag[t-1];
     if(!issued_all)begin
     ...
     if(v[6])begin
      ...
      butterfly_audit_valid<=1;butterfly_audit_stage<=tag[6].stage;butterfly_audit_index<=tag[6].bf;
      butterfly_audit_value<={next_ai,next_aq,next_bi,next_bq,new_saturations};
      if(tag[6].bf==10'd1023)begin
