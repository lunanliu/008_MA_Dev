...
module ota_cfo_chain (
 ...
 xpm_cdc_async_rst #(.DEST_SYNC_FF(4),.INIT_SYNC_FF(0),.RST_ACTIVE_HIGH(1))
  reset500(.src_arst(reset_request||cancel150),.dest_clk(clk500),.dest_arst(rst500));
 ...
 wire fail=error_code!=0||r1_fault||r2_fault||memory_error!=0||backend_bad;
 assign context_ready=!rst150&&!cancel150&&stage==IDLE&&!memory_busy&&!fw_busy&&!slow_busy;
 assign s_ready=stage==INGEST&&!fail?r1_sr:1'b0;
 ...
 cfo_rotate4 first_rotation(
  ...
  .s_valid(s_valid&&stage==INGEST&&!fail),.s_ready(r1_sr),.s_record(s_record),
 ...
 ota_async_fifo #(.WIDTH(135),.DEPTH(32)) window_samples(
  .wr_clk(clk150),.rd_clk(clk500),.reset_request(reset_request||cancel150||stage==CANCEL_DRAIN),
  ...
  .wr_busy(fw_busy),.rd_busy(fr_busy),.wr_count(),.rd_count(),.overflow(fw_over),.underflow(fr_under)
 );
 ...
 cfo_estimator_link observation_backend(
  .clk_fast(clk500),.clk_slow(clk150),.reset_async(reset_request),.abort_async(cancel150||stage==CANCEL_DRAIN),
  ...
  .m_fft_saturation_sum(),.fifo_write_count(),.fifo_read_count(),.fifo_full(),.fifo_empty(),.fifo_overflow(),.fifo_underflow(),
 ...
 always_ff @(posedge clk150)begin
  if(rst150)begin
   ...
   stage<=FAIL;
   if(error_code==0)error_code<=backend_bad?8'h60:(memory_error!=0?8'h20:(r1_fault?8'h30:8'h31));
  end else begin
