...
module cfo_fft256_core(
 ...
 logic [3:0] state;
 (* ram_style="block" *) logic [39:0] data_ram[0:255];
 ...
 always_ff @(posedge clk)begin
  ...
  if(rst || abort_sync)begin
   state<=LOAD;input_index<=0;output_index<=0;stage_index<=0;butterfly_index<=0;
   m_frame<=0;m_generation<=0;m_index<=0;m_last<=0;m_i<=0;m_q<=0;m_error<=0;m_saturations<=0;
   ...
   LOAD:if(s_valid)begin
    ...
    if((input_index!=0) && (s_frame!=m_frame || s_generation!=m_generation))begin
     state<=E_HOLD;m_error<=1;m_index<=0;m_last<=1;m_i<=0;m_q<=0;m_saturations<=0;
    end else if(s_index!=input_index || s_last!=(input_index==8'd255))begin
     state<=E_HOLD;m_error<=2;m_index<=0;m_last<=1;m_i<=0;m_q<=0;m_saturations<=0;
    end else begin
     data_ram[reverse8(input_index)]<={s_i,s_q};
     if(input_index==8'd255)begin stage_index<=0;butterfly_index<=0;state<=F_READ;end
     else input_index<=input_index+8'd1;
    end
   end
   F_READ:begin
    a_reg<=data_ram[addr_a];b_reg<=data_ram[addr_b];{wr,wi}<=twiddle_rom[twiddle_index];state<=F_MULT;
   end
   F_MULT:begin
    rr<=$signed(b_reg[39:20])*wr;ii<=$signed(b_reg[19:0])*wi;
    ri<=$signed(b_reg[39:20])*wi;ir<=$signed(b_reg[19:0])*wr;state<=F_ADD;
   end
   F_ADD:begin
   ...
   F_WRITE:begin
    data_ram[addr_a]<={next_ai,next_aq};data_ram[addr_b]<={next_bi,next_bq};
   ...
   O_READ:begin {m_i,m_q}<=data_ram[output_index];m_index<=output_index;m_last<=(output_index==8'd255);state<=O_HOLD;end
