...
module sfo_initial_fft_arbiter (
    input  logic         clk_125,
    input  logic         clk_500,
    input  logic         rst,
    input  logic         lease_valid,
    output logic         lease_ready,
    input  logic [ 31:0] lease_frame_id,
    ...
    output logic [ 12:0] accepted_input_beats,
    output logic [ 12:0] accepted_output_beats,
    output logic [  3:0] accepted_input_last,
    output logic [  3:0] accepted_output_last
);
  ...
  assign lease_ready = !rst && state == IDLE && core_s_ready && !core_m_valid && !core_error &&
      !error_sticky && !abort_request;
  assign s_ready = owned && !error_sticky && !core_error && !abort_request &&
      accepted_input_beats < 4096 && core_s_ready;
  ...
  sfo_initial_fft_service fft (
      .clk_125(clk_125),
      .clk_500(clk_500),
      .rst_n(core_rst_n),
      .s_valid(s_valid && owned && !error_sticky && !core_error && !abort_request &&
               accepted_input_beats < 4096),
      .s_ready(core_s_ready),
      .s_data(s_data),
      .s_transaction_end(s_last),
      .m_valid(core_m_valid),
      .m_ready(m_ready && owned && !abort_request && accepted_output_beats < 4096),
      .m_data(core_m_data),
      .m_transaction_end(core_m_last),
      .ingress_overflow(in_overflow),
      .ingress_underflow(in_underflow),
      .egress_overflow(out_overflow),
      .egress_underflow(out_underflow),
      .xfft_protocol_error(fft_protocol),
      .xfft_overflow(fft_overflow)
  );
  always_ff @(posedge clk_125) begin
    if (rst) begin
      ...
      accepted_output_beats <= 0;
      accepted_input_last <= 0;
      accepted_output_last <= 0;
    end else begin
      abort_done <= 0;
      case (state)
        ...
        IDLE: begin
          if (lease_valid && lease_ready) begin
            state <= OWNED;
            active_frame_id <= lease_frame_id;
            accepted_input_beats <= 0;
            accepted_output_beats <= 0;
            accepted_input_last <= 0;
            accepted_output_last <= 0;
        ...
        OWNED: begin
          ...
          if (core_error || (core_m_valid && accepted_output_beats >= 4096) ||
              (s_valid && accepted_input_beats >= 4096))
            error_sticky <= 1;
          if (lease_release) begin
            if (accepted_input_beats == 4096 && accepted_output_beats == 4096 && accepted_input_last
                == 8 && accepted_output_last == 8 && !core_m_valid && !core_error && !error_sticky)
              state <= IDLE;
            else error_sticky <= 1;
          end
