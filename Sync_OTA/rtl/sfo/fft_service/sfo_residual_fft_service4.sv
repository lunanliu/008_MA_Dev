...
module sfo_residual_fft_service4 #(
  ...
  always_ff @(posedge clk_500) begin
    if (reset500) begin
      ...
      if (in_fifo_underflow) ingress_underflow_500 <= 1'b1;
  ...
  always_ff @(posedge clk_125) begin
    if (!rst_n) begin
      ...
      errors_meta <= {
        egress_overflow_500, ingress_underflow_500, xfft_protocol_error_500, xfft_overflow_500
      };
      errors_sync <= errors_meta;
      status_sync <= {status_sync[0], status_toggle};
      status_last <= status_sync[1];
    end
  end
endmodule
