...
module sfo_initial_derotate_wide_4lane (
    input  logic         clk,
    input  logic         rst,
    input  logic         s_valid,
    ...
    input  logic [ 71:0] s_cos,
    input  logic [ 71:0] s_sin,
    input  logic [ 31:0] s_tag,
    output logic         m_valid,
    input  logic         m_ready,
    output logic [147:0] m_i,
    output logic [147:0] m_q,
  ...
  generate
    for (lane = 0; lane < 4; lane = lane + 1) begin : g_lane
      ...
      wire [47:0] coefficient_data = {
        {6{coefficient_im[17]}}, coefficient_im, {6{coefficient_re[17]}}, coefficient_re
      };
      t05_cmpy_rotate_18x18 u_rotate (
          ...
          .s_axis_b_tdata    (coefficient_data),
          .m_axis_dout_tvalid(cmpy_valid[lane]),
          .m_axis_dout_tdata (cmpy_data[lane])
      );
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      metadata_valid <= {metadata_valid[2:0], accept};
      tag_pipe[0] <= s_tag;
      ...
      endcase
      if ((!(|reset_guard) && cmpy_valid != {4{metadata_valid[3]}}) || fifo_overflow ||
