...
module sfo_initial_five_sum_accumulator (
    input  logic               clk,
    input  logic               rst,
    ...
    input  logic signed [10:0] s_bin,
    input  logic        [17:0] s_weight,
    input  logic signed [47:0] s_phase,
    input  logic               s_last,
    output logic               m_valid,
    input  logic               m_ready,
    output logic        [31:0] m_frame_id,
    output logic        [29:0] m_s0,
    output logic signed [40:0] m_s1,
    output logic        [47:0] m_s2,
    output logic signed [75:0] m_t0,
    output logic signed [84:0] m_t1,
    output logic               m_error,
    output logic        [ 2:0] m_error_code,
    output logic        [12:0] accepted_count,
    output logic        [12:0] accumulated_count
);
  ...
  assign expected_bin = (expected_bin_index < 820) ? $signed(
      expected_bin_index + 11'd1
  ) : $signed(
      {1'b0, expected_bin_index}
  ) - 12'sd1640;
  assign input_error = s_pair != expected_pair || s_bin != expected_bin || s_weight < 18'd1 ||
      s_weight > 18'd131072 || s_phase < -48'sd35184372088832 || s_phase > 48'sd35184372088831 ||
      s_last != (accepted_count == 13'd6559);
  t06_fs_mult_u18_s11 u_wk (
      .CLK (clk),
      .SCLR(clear_ip),
      .A   (s_weight),
      .B   (s_bin),
  ...
  always_comb begin
    ...
    sum_error = sum0_next[30] || sum1_next[41] != sum1_next[40] || sum2_next[48] || wkk[39] ||
        total0_next[76] != total0_next[75] || total1_next[85] != total1_next[84];
  end
  integer stage;
  always_ff @(posedge clk) begin
    if (rst || start_fire) begin
      ...
      weight_pipe[0] <= s_weight;
      bin_pipe[0] <= s_bin;
      phase_pipe[0] <= s_phase;
      wk_pipe[0] <= wk;
      wy_pipe[0] <= wy;
      for (stage = 1; stage < PIPE_LATENCY; stage = stage + 1)
      weight_pipe[stage] <= weight_pipe[stage-1];
      for (stage = 1; stage < MULT_LATENCY; stage = stage + 1) begin
        bin_pipe[stage] <= bin_pipe[stage-1];
        phase_pipe[stage] <= phase_pipe[stage-1];
        wk_pipe[stage] <= wk_pipe[stage-1];
        wy_pipe[stage] <= wy_pipe[stage-1];
      end
      ...
      if (accept) begin
        ...
        if (expected_bin_index == 11'd1639) begin
          expected_bin_index <= 0;
          expected_pair <= expected_pair + 2'd1;
        end else expected_bin_index <= expected_bin_index + 11'd1;
      end
      if (busy && !m_valid && valid_pipe[5]) begin
        m_s0 <= sum0_next[29:0];
        m_s1 <= sum1_next[40:0];
        m_s2 <= sum2_next[47:0];
        m_t0 <= total0_next[75:0];
        m_t1 <= total1_next[84:0];
        accumulated_count <= accumulated_count + 13'd1;
        if (accumulated_count == 13'd6559) begin
          m_valid <= 1;
          m_error <= 0;
          m_error_code <= 0;
        end
      ...
      if ((accept && input_error) || (busy && !m_valid && valid_pipe[5] && sum_error)) begin
        m_valid <= 1;
        m_error <= 1;
        m_error_code <= (accept && input_error) ? 3'd6 : 3'd4;
        valid_pipe <= 0;
        m_s0 <= 0;
        m_s1 <= 0;
        m_s2 <= 0;
