...
module sfo_initial_unsigned_divider_service (
    input  logic        clk,
    input  logic        rst,
    input  logic        s_valid,
  ...
  logic
      result_full, result_empty, result_wr_busy, result_rd_busy, result_overflow, result_underflow;
  ...
  assign
      ready_storage = !rst && !meta_wr_busy && !meta_rd_busy && !result_wr_busy && !result_rd_busy;
  ...
  assign retire = !rst && divider_valid && !meta_empty && !meta_rd_busy && !result_wr_busy &&
      !result_full;
  ...
  t04_div_u63_u47 divider (
      ...
      .s_axis_divisor_tdata  ({1'b0, s_divisor}),
      .s_axis_dividend_tvalid(issue),
      .s_axis_dividend_tready(dividend_ready),
      .s_axis_dividend_tdata ({1'b0, s_dividend}),
      .m_axis_dout_tvalid    (divider_valid),
      .m_axis_dout_tuser     (divider_user),
      .m_axis_dout_tdata     (divider_data)
  );
  ...
  sfo_initial_unsigned_divider_service_fifo #(
      ...
      .full     (result_full),
      .empty    (result_empty),
      .wr_busy  (result_wr_busy),
      .rd_busy  (result_rd_busy),
  ...
  assign container_error = !expected_zero &&
      ((divider_data[111] != quotient[62]) || (divider_data[47] != remainder[46]) ||
       remainder >= meta_head.divisor);
  assign increment = (twice_remainder > denominator) ||
      (twice_remainder == denominator && quotient[0]);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if ((ready_storage && s_valid && divisor_ready != dividend_ready) ||
          (divider_valid && (!retire || flag_mismatch || container_error)) || meta_overflow ||
          meta_underflow || result_overflow || result_underflow || pending_count > 32 ||
          (consume && pending_count == 0))
  ...
  always @(posedge clk) begin
    ...
    if (rst) last_issue_cycle = -100;
      ...
      if (issue) begin
        if (simulation_cycle - last_issue_cycle < 8)
          $fatal(1, "DIV official issue interval below CPD8");
      ...
      if (retire && ((divider_user[0] !== expected_zero) || container_error))
        $fatal(
            1,
            "DIV vendor flag/container/remainder mismatch tag=%0d raw=%h user=%b divisor=%h",
            meta_head.tag,
            divider_data,
            divider_user,
            meta_head.divisor
        );
      if (pending_count > 32 || (consume && pending_count == 0))
        $fatal(1, "DIV final credit invariant");
    end
  end
