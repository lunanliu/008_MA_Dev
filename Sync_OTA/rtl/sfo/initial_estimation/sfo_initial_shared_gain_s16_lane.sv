`include "initial_shared_gain_ip_config.svh"
...
module sfo_initial_shared_gain_s16_lane (
    input  logic               clk,
    input  logic               rst,
    input  logic               ce,
  ...
  localparam integer MULT_LATENCY = `SFO_INITIAL_SG_MULT_LATENCY;
  ...
  function automatic logic [15:0] signed_magnitude16(input logic [15:0] magnitude,
  ...
  always_comb begin
    ...
    input_metadata.s18_overflow = (provisional_i < -21'sd131072) || (provisional_i > 21'sd131071) ||
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      mult_valid <= '0;
      for (stage = 0; stage < MULT_LATENCY; stage = stage + 1) mult_metadata[stage] <= '0;
  ...
  t06_sg_div_u53_u37 u_ratio (
      ...
      .m_axis_dout_tdata     (div_data_out),
  ...
  always_comb begin
    ...
    increment = (remainder > half_other) || ((remainder == half_other) && quotient[0]);
    rounded_magnitude = {1'b0, quotient} + {{53{1'b0}}, increment};
    other_negative = div_metadata_out.limiting_i ? div_metadata_out.negative_q :
        div_metadata_out.negative_i;
    other_capacity = other_negative ? 16'd32768 : 16'd32767;
    arithmetic_error = div_metadata_out.scaled &&
        ((div_metadata_out.denominator == 0) || (remainder >= div_metadata_out.denominator) ||
         (rounded_magnitude > {38'd0, other_capacity}) || (div_data_out[95:93] != 0) ||
         (div_data_out[39:37] != 0));
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      out_scaled <= 1'b0;
      out_limiting_i <= 1'b0;
      out_s18_overflow <= 1'b0;
      out_range_error <= 1'b0;
    end else if (ce) begin
