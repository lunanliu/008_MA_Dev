...
module sfo_initial_pair_phasor_bfp (
    input  logic               clk,
    input  logic               rst,
    input  logic               s_valid,
    ...
    input  logic               m_ready,
    output logic signed [32:0] m_phasor_re,
    output logic signed [32:0] m_phasor_im,
    output logic signed [47:0] m_bfp_re,
    output logic signed [47:0] m_bfp_im,
    output logic        [ 5:0] m_shift,
    output logic               m_zero,
  ...
  assign cmpy_a = {{7{first_conjugate_im[16]}}, first_conjugate_im, {7{first_re[16]}}, first_re};
  ...
  t04_cmpy_17x16 u_cmpy (
      .aclk              (clk),
      .s_axis_a_tvalid   (accept),
      .s_axis_a_tdata    (cmpy_a),
      .s_axis_b_tvalid   (accept),
      .s_axis_b_tdata    (s_second_iq),
      .m_axis_dout_tvalid(cmpy_valid),
      .m_axis_dout_tdata (cmpy_data)
  );
  integer stage;
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      case ({
        accept, release_credit
      })
        2'b10:   pending_count <= pending_count + 6'd1;
        2'b01:   pending_count <= pending_count - 6'd1;
        default: pending_count <= pending_count;
      endcase
      if ((!(|reset_guard) && (cmpy_valid != metadata_valid[CMPY_LATENCY-1])) || fifo_overflow ||
          fifo_underflow || (fifo_write && fifo_full))
  ...
  always_comb begin
    ...
    fifo_input.phasor_re = wide_re[32:0];
  ...
  assign m_phasor_re = fifo_output.phasor_re;
  assign m_phasor_im = fifo_output.phasor_im;
  assign m_bfp_re = fifo_output.bfp_re;
  assign m_bfp_im = fifo_output.bfp_im;
  assign m_shift = fifo_output.shift;
  assign m_zero = fifo_output.zero_phasor;
  assign m_error = fifo_output.range_error;
`ifndef SYNTHESIS
  always @(posedge clk) begin
    if ((rst || fifo_wr_busy) && fifo_write) $fatal(1, "T06 pair/BFP write during reset/busy");
    if ((rst || fifo_rd_busy) && release_credit) $fatal(1, "T06 pair/BFP read during reset/busy");
    if (!rst) begin
