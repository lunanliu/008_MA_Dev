`timescale 1ns / 1ps
module sfo_initial_frame_context_join #(
    ...
    output logic signed                                [31:0] m_coarse_cfo_hz,
    output logic signed                                [31:0] m_fine_start,
    output logic                                              m_numeric_valid,
    output logic                                              m_fail_close,
    output logic                                              m_approved_outage,
    output logic                                              m_protocol_fault,
    output logic                                              halted,
    output logic                                       [ 7:0] fault_reason,
    output logic                                       [31:0] fault_expected_id,
    output logic                                       [31:0] fault_observed_id,
    output logic                                       [ 1:0] fault_source_mask,
    output logic                                       [ 2:0] canceled_cfo,
    output logic                                       [ 2:0] canceled_fine,
    output logic                                       [ 1:0] cfo_occupancy,
    output logic                                       [ 1:0] fine_occupancy,
  ...
  always_comb begin
    ...
    if (take_cfo && s_cfo.status[15:12] != 2) begin
      fault_now = 1;
      reason_now = 1;
      expected_now = seeded ? next_cfo : seed_now;
      observed_now = s_cfo.frame_id;
      source_now = 1;
    end else if (take_fine && s_fine.status[15:12] != 3) begin
      fault_now = 1;
      reason_now = 1;
      expected_now = seeded ? next_fine : seed_now;
      observed_now = s_fine.frame_id;
      source_now = 2;
    end else if (!seeded && take_cfo && take_fine && s_cfo.frame_id != s_fine.frame_id) begin
      fault_now = 1;
      reason_now = 4;
      expected_now = seed_now;
      observed_now = s_fine.frame_id;
      source_now = 2;
    end else if (seeded && take_cfo && s_cfo.frame_id != next_cfo) begin
      fault_now = 1;
      reason_now = (s_cfo.frame_id == (next_cfo - 32'd1)) ? 2 : 3;
      expected_now = next_cfo;
      observed_now = s_cfo.frame_id;
      source_now = 1;
    end else if (seeded && take_fine && s_fine.frame_id != next_fine) begin
      fault_now = 1;
      reason_now = (s_fine.frame_id == (next_fine - 32'd1)) ? 2 : 3;
      expected_now = next_fine;
      observed_now = s_fine.frame_id;
      source_now = 2;
    ...
    joined_status = normal_pair ?
        16'h4800 : (approved_pair ? 16'h4402 : invalid_status(cfo_head.status, fine_head.status));
  end
    ...
    if (rst) begin
      ...
      if (!halted) begin
        if (fault_now) begin
          halted <= 1;
          fault_pending <= 1;
          pending_fault_id <= fault_frame_now;
          pending_fault_status <= (reason_now == 5) ? 16'h4056 : 16'h4036;
          fault_reason <= reason_now;
          fault_expected_id <= expected_now;
          fault_observed_id <= observed_now;
          fault_source_mask <= source_now;
          ...
          if (unmatched && !partner_now) age <= age + 1'b1;
          else age <= 0;
          ...
          if (take_cfo) begin
            cfo_fifo[cfo_wr] <= s_cfo;
            cfo_wr <= !cfo_wr;
          end
          if (take_fine) begin
            fine_fifo[fine_wr] <= s_fine;
            fine_wr <= !fine_wr;
          end
          case ({
            take_cfo, do_join
