...
module sfo_farrow_add #(
  ...
  generate
    if (W == 18) begin : g18
      ...
      initial $fatal(1, "Unsupported official C_AddSub width");
    end
  endgenerate
endmodule
module sfo_farrow_horner #(
    ...
    input  wire                clk,
    input  wire                reset,
    input  wire signed [G-1:0] a,
    input  wire signed [G-1:0] branch,
    input  wire        [M-1:0] mu,
  ...
  always @(posedge clk)
    if (!reset && !$isunknown(sum64)) begin
...
module sfo_farrow_real #(
    parameter integer C = 14,
    D = 16,
    M = 8,
    parameter integer G = D + 3,
    W = D + C + 2,
    T = D + C - 3,
    P = G + M
) (
    input  wire                    clk,
    input  wire                    reset,
    input  wire signed [    D-1:0] xa,
    input  wire signed [    D-1:0] xb,
    input  wire signed [    D-1:0] xc,
    input  wire signed [    D-1:0] xd,
    input  wire        [    M-1:0] mu,
  ...
  assign raw[0] = raw0_hold;
  assign raw[1] = raw1_ip;
  assign raw[2] = raw2_hold;
  assign raw[3] = raw3_hold;
  ...
  always @(posedge clk) begin
    xbp <= xb;
    xc_first <= {{2{xc[D-1]}}, xc};
    bc_delay[0] <= bc;
    ca_delay[0] <= ca;
    sec_delay[0] <= second_difference;
    sec_delay[1] <= sec_delay[0];
    b_delay[0] <= xbp;
    for (i = 1; i < 3; i = i + 1) begin
      bc_delay[i] <= bc_delay[i-1];
      ca_delay[i] <= ca_delay[i-1];
      b_delay[i]  <= b_delay[i-1];
    end
    r2_first  <= raw2_wide[W-1:0];
    r0_first  <= raw0_wide[W-1:0];
    ca_first  <= ca64 <<< (C - 3);
    raw0_hold <= r0_first;
    raw2_hold <= r2_first;
    raw3_hold <= r3_first;
    for (i = 0; i < 4; i = i + 1) begin
      rounded_branch = rne_branch(raw[i]);
      branch[i] <= saturate_branch(rounded_branch);
      branch_sat[i] <= (rounded_branch > ((64'sd1 << (G - 1)) - 1)) ||
          (rounded_branch < -(64'sd1 << (G - 1)));
    end
    mu_delay[0] <= mu;
    for (i = 1; i < 17; i = i + 1) mu_delay[i] <= mu_delay[i-1];
    b1_delay[0] <= branch[1];
    for (i = 1; i < 5; i = i + 1) b1_delay[i] <= b1_delay[i-1];
    b0_delay[0] <= branch[0];
    for (i = 1; i < 10; i = i + 1) b0_delay[i] <= b0_delay[i-1];
    final_sat <= (h0_64 > OUTPUT_HI) || (h0_64 < OUTPUT_LO);
    if (h0_64 > OUTPUT_HI) y <= OUTPUT_HI[D-1:0];
    else if (h0_64 < OUTPUT_LO) y <= OUTPUT_LO[D-1:0];
    else y <= h0[D-1:0];
  end
  ...
  genvar j;
  generate
    for (j = 0; j < 4; j = j + 1) begin : trace_branch
  ...
  always @(posedge clk)
    if (!reset) begin
      if (!$isunknown(
              ...
              {{(64 - W) {raw3_wide[W-1]}}, raw3_wide[W-1:0]}
          ) !== raw3_wide)
        $fatal(1, "raw3 typed overflow");
      if (!$isunknown(
              raw2_wide
          ) && $signed(
              {{(64 - W) {raw2_wide[W-1]}}, raw2_wide[W-1:0]}
          ) !== raw2_wide)
        $fatal(1, "raw2 typed overflow");
      if (!$isunknown(
              raw1_wide
          ) && $signed(
              {{(64 - W) {raw1_wide[W-1]}}, raw1_wide[W-1:0]}
          ) !== raw1_wide)
...
module sfo_farrow_parallel #(
  ...
  assign output_valid = valid_pipe[22];
  assign output_tag   = tag_pipe[22];
  integer i;
  always @(posedge clk) begin
    if (reset) begin
      valid_pipe <= 0;
      for (i = 0; i < 23; i = i + 1) tag_pipe[i] <= 0;
    end else begin
      valid_pipe  <= {valid_pipe[21:0], input_valid};
      tag_pipe[0] <= input_tag;
      for (i = 1; i < 23; i = i + 1) tag_pipe[i] <= tag_pipe[i-1];
    end
  end
  function automatic integer node_stage(input integer n);
    begin
      if (n < 4) node_stage = 5;
      else if (n < 8) node_stage = 6;
      else
        case (n)
          8: node_stage = 9;
          9: node_stage = 10;
  ...
  generate
    for (r = 0; r < REALS; r = r + 1) begin : real_channel
      sfo_farrow_real #(
          ...
          .clk       (clk),
          .reset     (reset),
          .xa        (input_data[(r*4+0)*D+:D]),
          .xb        (input_data[(r*4+1)*D+:D]),
          .xc        (input_data[(r*4+2)*D+:D]),
          .xd        (input_data[(r*4+3)*D+:D]),
          .mu        (input_data[REALS*4*D+(r/2)*M+:M]),
          .trace     (trace_data[r*18*64+:18*64]),
          .saturation(saturation[r*18+:18]),
          .y         (output_data[r*D+:D])
      );
    end
