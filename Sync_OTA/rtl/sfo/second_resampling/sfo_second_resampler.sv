...
module sfo_second_resampler #(
    ...
    // Formal descriptor transfer is coupled to the actual engine configuration.
  ...
  assign m_nominal_first = engine_beat == 0;
  assign m_nominal_last = engine_beat == NOMINAL_SAMPLES / 4 - 1;
  ...
  sfo_second_pass_descriptor #(
      ...
      .s_t09_result     (cfg_t09_result),
      .s_source_complete(cfg_source_complete),
      .s_source_error   (cfg_source_error),
      ...
      .m_valid          (d_valid),
      .m_ready          (state == LAUNCH && engine_cfg_ready && context_ack && !abort_request),
      .m_error          (d_error),
      .m_frame_id       (d_frame),
      .m_generation     (d_gen),
      .m_step           (d_step),
  ...
  sfo_first_frame_scheduler #(
      .SAMPLE_COUNT(TARGET),
      .INPUT_BEATS((NOMINAL_SAMPLES + 72) / 4),
      .PROCESSING_LIMIT_CYCLES(PROCESSING_LIMIT_CYCLES)
  ) engine (
      .clk                  (clk),
      .rst                  (rst),
      .abort_request        (abort_engine),
      .cfg_valid            (state == LAUNCH && !abort_request && context_ack),
    ...
    if (rst) begin
      ...
      if (abort_request && (state == PREPARE || state == LAUNCH)) begin
        ...
        case (state)
          ...
          LAUNCH: if (engine_cfg_ready && context_ack) state <= ACTIVE;
          ACTIVE:
