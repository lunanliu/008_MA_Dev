...
module sfo_second_pass_descriptor #(
    ...
    input  logic                s_source_complete,
    input  logic        [  7:0] s_source_error,
  ...
  always_comb begin
    ...
    if (s_frame_id != s_estimate_frame_id || s_generation != s_estimate_generation)
      input_error = 8'h40;
             ...
             s_t09_result[95:89] != 74 || s_t09_result[88:82] != 74)
      input_error = 8'h41;
      ...
      input_error = 8'h44;
    else if (s_nominal_first < 0 || s_nominal_first > 1336320 - NOMINAL_SAMPLES)
      input_error = 8'h45;
    ...
    if (rst) begin
      ...
      case (state)
        ...
        PREP: begin
          ...
          div_remainder <= 0;
          bit_count <= 0;
          coordinate_term <= 64'sd53 - (raw64 <<< 2);
          state <= COORD;
        end
        COORD: begin
          phase_bias <= coordinate_term <<< 28;
          state <= DIVIDE;
        end
        DIVIDE: begin
        ...
        CHECK: begin
          if (m_step < 268434651 || m_step > 268436261 || m_step != estimate_step || m_phase < 0 ||
              phase_last < m_phase || first_base - 64'sd107 < 0 ||
              last_base + 64'sd2 >= 4 * INPUT_COUNT)
            m_error <= 8'h47;
          state <= HOLD;
        end
  ...
  initial
    if (NOMINAL_SAMPLES != 1336320 && NOMINAL_SAMPLES != 5120)
      $fatal(1, "T10 production or explicitly short module test only");
