...
module ota_capture_controller #(
 ...
 assign start_ready=!rst125&&stage==IDLE&&!cfo_busy&&bridge_idle&&bridge_error==0;
 ...
 wire fault=bridge_error!=0||mem_error!=0||desc_error!=0||algorithm_error||timed_out;
 ...
 always_ff @(posedge clk125)begin
  if(rst125)begin
   ...
   if(active&&stage!=CANCEL_DRAIN&&stage!=CANCEL_RELEASE&&(cancel||fault))begin
    stage<=CANCEL_DRAIN;
    if(error_code==0)error_code<=cancel?8'h01:(bridge_error!=0?8'h72:(mem_error!=0?8'h23:(desc_error!=0?desc_error:(algorithm_error?8'h50:8'h51))));
   end else case(stage)
    IDLE:if(start_valid&&start_ready)begin
     ...
     if(start_words==0||start_words>MAX_CAPTURE_WORDS||start_base_word+reserved_words<start_base_word)begin error_code<=8'h10;stage<=COMPLETE;end
     else stage<=OPEN;
    end
