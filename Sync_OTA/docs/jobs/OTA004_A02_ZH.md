# OTA004/A02：完整真实工程、短顶层控制与一次核心综合

## 范围与配对

PROJECT_SPEC已授权完整系统、必要短检查、核心综合与手工上板交付。本包由Astra 01a0ac17-f298-7201-848c-58d09e90ebb4派唯一Luna 01a0ac17-4f21-7be3-8690-540c11497b1e；必须先取得总管家新的OTA004/A02具体grant。OTA001～003授权均已归还，不得复用。
...
输入清单OTA004_A02_source_lock.json逐项记录长度/SHA256。127项生产RTL、135个唯一module/package定义、45个唯一真实IP；生产CFO只选ota_cfo_chain_a02.sv，不能把旧A01同名module一并加入。A02补失败后端结果留存和既有FIFO诊断；root首错/监测域/完成保持见IMPLEMENTATION_A05_ZH.md。OTA002/003共69项旧冻结输入已重新核对未改。
...
入口tools/vivado/run_ota004_a02.tcl；监管tools/monitor_ota004.ps1继承已验证PID+创建时间树逻辑，只把启动可用物理内存线提高到12GiB。每阶段新目录work/OTA004/<stage>_a02，失败另编号，成功阶段不重跑。

具体grant通过并新建独立目录后，示例：
powershell.exe -NoProfile -ExecutionPolicy Bypass -File D:/008_MA_Dev/Sync_OTA/tools/monitor_ota004.ps1 -Stage prepare -Attempt D:/008_MA_Dev/Sync_OTA/work/OTA004/prepare_a02 -Source D:/008_MA_Dev/Sync_OTA/tools/vivado/run_ota004_a02.tcl -HardSeconds 2400
...
总管家在A01准入前发现3份必需头文件漏拷；A01未启动，原入口/检查器/清单/任务书原样保留，禁止执行旧A01。A02仅修复独立工程头文件闭合，不改RTL算法、TB、IP参数、阈值、并行/内存/时间预算。仍须新的A02具体grant，12GiB启动门槛保持。
...
A02入口run_ota004_a02.tcl和检查器verify_ota004_a02_project.py另存新文件。实际工程每次导出actual_headers.tsv及actual_include_dirs.tsv，并核对3成员、FILE_TYPE、综合/仿真可见性、两个文件集唯一搜索路径；扫描精确127生产RTL、指定TB和6份XPM中的include递归依赖，拒绝缺失、歧义、动态未审查引用或未锁定头文件。独立记录头文件长度/SHA256和引用边，不能以compile_order可能不单列头文件为由漏审。全部A02冻结输入每阶段核验；实际IP/模块/XPM门槛沿用A01。
