# OTA004/A04：完整真实工程、短顶层控制与一次核心综合

## 范围与配对

PROJECT_SPEC已授权完整系统、必要短检查、核心综合与手工上板交付。本包由Astra 01a0ac17-f298-7201-848c-58d09e90ebb4派唯一Luna 01a0ac17-4f21-7be3-8690-540c11497b1e；必须先取得总管家新的OTA004/A04具体grant。OTA001～003授权均已归还，不得复用。
...
输入清单OTA004_A04_source_lock.json逐项记录长度/SHA256。IP入口改为ip/sources_a04.f，只选ip/config_a04中的45个保真副本。127项生产RTL、135个唯一module/package定义、45个唯一真实IP；生产CFO只选ota_cfo_chain_a02.sv，不能把旧A01同名module一并加入。A02补失败后端结果留存和既有FIFO诊断；root首错/监测域/完成保持见IMPLEMENTATION_A05_ZH.md。OTA002/003共69项旧冻结输入已重新核对未改。
...
入口tools/vivado/run_ota004_a04.tcl；监管tools/monitor_ota004_a03.ps1继承已验证PID+创建时间树逻辑，按2026-09-17用户最新指令将12GiB作为告警参考，不作为启动拒绝线，并记录系统提交余量、分页速率、分页文件及完整子树内存/CPU/IO与工具日志进展。每阶段新目录work/OTA004/<stage>_a04，失败另编号，成功阶段不重跑。

具体grant通过并新建独立目录后，示例：
powershell.exe -NoProfile -ExecutionPolicy Bypass -File D:/008_MA_Dev/Sync_OTA/tools/monitor_ota004_a03.ps1 -Stage prepare -Attempt D:/008_MA_Dev/Sync_OTA/work/OTA004/prepare_a04 -Source D:/008_MA_Dev/Sync_OTA/tools/vivado/run_ota004_a04.tcl -HardSeconds 2400

smoke阶段同样2400秒；synth阶段7200秒。各阶段实际HardSeconds不得超过包级剩余预算。首启记录后180分钟为全包硬截止，贯穿包装重试。保存每条实际命令/私有修复哈希和绝对UTC期限。

prepare在根目录创建完整XPR，用import_ip生成Sync_OTA.srcs受管副本，原ip/config及ip/config_a04冻结XCI保持不变。重复进入只打开已有XPR，不删除重建、不force已成功目标。smoke/synth均打开这一XPR并重新审核part/top/源集；每阶段保存actual_sources.tsv、actual_ips.tsv、真实编译成员哈希。失败的可变.sim日志/PRJ/bat/Tcl归档到本次attempt，禁止为补记录重跑成功计算。
...
## A04：修复XCI移植及真实TSV尾空列，延续原预算

A03 prepare原生失败：5个IP无法创建，实际只有40个IP对象；XML通用解析/写回将厂商boundaryDescriptionJSON属性中的物理换行转成字符间空格，损坏名称/类型等元数据，45项均受此转换影响。标量IP参数对比无法发现这类损坏，A03不得判PASS。独立审计的strip又吞掉约束行尾空字段，192行实际TSV中1行仅被误读为4列。A03完整树00:45:05.1133386Z关闭，退出1，未超时；没有进入generate_target/Wrapper/smoke/synth。证据reports/OTA004/A03_FAILURE_ASTRA_REVIEW.json。

A04所有45个XCI从先前已锁定且逐字节保存的原件建立新ip/config_a04；不再XML序列化。只精确替换既定RUNTIME_PARAM.OUTPUTDIR与PROJECT_PARAM.PREFHDL标量文本，所有其他字节（含厂商跨行JSON、命名空间）保持原样；逆替换两标量可恢复全部原始字节。45份JSON按厂商物理行拼接后均可解析，实际工程导出受管XCI时除既有标量身份检查，还需比较完整boundary JSON对象，不放宽45IP/0黑盒/真实模块门槛。来源和字节证据见docs/provenance/OTA004_A04_IP_SOURCE_LOCK.json。

根XPR已有全部45个XCI文件引用，但5个IP实例加载失败。继续同一工程，不删除或重建。新入口在prepare且已有XPR时先调用tools/repair_ota004_a04_managed.py：严格核对docs/jobs/OTA004_A04_REPAIR_BASELINE.json，封存原XPR+45受管XCI到本attempt/failed_a03_project_before_repair并逐项验证，再只替换这45个失败阶段受管副本；不改根XPR、旧冻结输入或原3工程。修复状态/回执在work/OTA004，全部归档核对完才开始替换；中断只续必要部分，回执完成后重入不得重写工具已生成的成功目标。发现已存在综合DCP或非预期编辑即停止交Astra，不覆盖。随后open_project并审核45真实IP；原40项导入不构成计算成功阶段，不重跑任何已完成计算。

新检查器verify_ota004_a04_project.py读取TSV保留每个tab及尾部空列；使用A03真实192行静态回归，5列全部保留，约束行空sim属性合法；生产RTL/IP/头文件的严格门槛保持。A03旧入口/检查器/210项输入不改。监管继续使用monitor_ota004_a03.ps1，12GiB只告警，不新增内存硬门槛。

首启时间已发生于2026-09-17 00:44:33.0452158 UTC，整个OTA004包绝对硬截止仍为2026-09-17 03:44:33.0452158 UTC（本地05:44:33 CEST），新修订/包装重试不重新起180分钟。prepare/smoke各最多2400秒、synth最多7200秒，实际传值仍不得超过这个绝对截止剩余秒数。范围仍只重试未完成prepare，再smoke、synth；无算法变更、无新实验组、无全帧/实现/板测。需要总管家对新A04输入签发具体修订准入后唯一Luna执行。

### 四个FIR系数MIF生成依赖

原配置中t07_g3_up47/down47/up15/down15各有一个MODELPARAM_VALUE.C_COEF_FILE相对MIF名（共4项）；系数值在同一冻结XCI内，MIF须由本工程generate_target生成，不能靠参数相同比对假称实际文件已经闭合。A04审计显式区分generate_target前后：生成前仅静态参数身份；生成后从实际Vivado get_files -all导出MIF成员，要求4个精确文件名全部存在、非空、位于本工程且同名多份哈希一致，记录真实路径/长度/SHA256。smoke在launch_simulation后、run all前再次核查xsim工作目录4份MIF与实际生成件哈希一致；缺失/异物即失败，不从旧工程借文件、不默默跳过。头文件、编译源及45IP/边界JSON审计继续执行。
