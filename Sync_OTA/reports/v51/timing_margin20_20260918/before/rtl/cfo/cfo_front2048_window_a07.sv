...
module cfo_front2048_window(
 ...
 wire [2:0] coefficient_index0=coefficient_return_page ? coefficient_page1_data : coefficient_page0_data;
 logic [5:0] v;
 ...
 always_ff @(posedge clk)begin
  ...
  c_pipe[1]<=coefficient_table[coefficient_index0];
  for(data_j=2;data_j<6;data_j=data_j+1)c_pipe[data_j]<=c_pipe[data_j-1];
  begin {ar2,ai2}<=f_pipe[1];{cr2,ci2}<=c_pipe[1];end
 ...
 always_ff @(posedge clk)begin
  ...
  if(rst || abort_sync)begin
   state<=FEED;first_pending<=1;coefficient_base<=0;address_valid<=0;coefficient_read_command<=0;v<=0;fft_done<=0;sum_done<=0;sum_i<=0;sum_q<=0;pilot_count<=0;
   ...
   if(fft_m_valid && fft_m_ready && f_error!=0)begin
    state<=HOLD;pilot_audit_valid<=0;m_error<=f_error;m_z_i<=0;m_z_q<=0;m_pilot_count<=0;m_fft_saturations<=0;address_valid<=0;coefficient_read_command<=0;v<=0;
   end
   if(state==HOLD && m_ready)begin
    state<=FEED;first_pending<=1;address_valid<=0;coefficient_read_command<=0;v<=0;fft_done<=0;sum_done<=0;sum_i<=0;sum_q<=0;pilot_count<=0;
   end
  end
 end
endmodule
