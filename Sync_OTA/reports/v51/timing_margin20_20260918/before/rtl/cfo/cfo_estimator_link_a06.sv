...
module cfo_estimator_link(
 ...
 // The system reset still stops both interfaces immediately. Only fifo_reset
 // drives XPM_FIFO_ASYNC.rst: both transitions pass through four clk_fast FFs.
 // Once captured, rst_fast holds a common_reset pulse for four fast edges.
 // INIT=1 starts the FIFO in reset; INIT_SYNC_FF=1 also models GSR in simulation.
 wire fifo_reset;
 xpm_cdc_sync_rst #(.DEST_SYNC_FF(4),.INIT(1),.INIT_SYNC_FF(1),.SIM_ASSERT_CHK(1))
  fifo_reset_sync(.src_rst(rst_fast),.dest_clk(clk_fast),.dest_rst(fifo_reset));

 // XPM 2021.1 performs two internal reset passes. Do not release the reader
 // during a gap in rd_busy: first observe FIFO reset+wr_busy, then wait for
 // the complete writer reset sequence, release the reader, and return its ACK.
 // These reset-only levels are independent of the per-frame credit toggle.
 logic fifo_reset_seen=1'b0,fifo_reset_done=1'b0,slow_reset_ready=1'b0;
 wire fifo_reset_done_slow,slow_reset_ready_fast;
 always_ff @(posedge clk_fast or posedge rst_fast)begin
  if(rst_fast)begin fifo_reset_seen<=0;fifo_reset_done<=0;end
  else begin
   if(fifo_reset && wr_busy)fifo_reset_seen<=1;
   if(fifo_reset_seen && !fifo_reset && !wr_busy)fifo_reset_done<=1;
  end
 end
 xpm_cdc_single #(.DEST_SYNC_FF(2),.INIT_SYNC_FF(1),.SRC_INPUT_REG(0),.SIM_ASSERT_CHK(1))
  reset_done_to_slow(.src_clk(clk_fast),.src_in(fifo_reset_done),.dest_clk(clk_slow),.dest_out(fifo_reset_done_slow));
 always_ff @(posedge clk_slow or posedge rst_slow)begin
  if(rst_slow)slow_reset_ready<=0;
  else slow_reset_ready<=fifo_reset_done_slow && !rd_busy;
 end
 xpm_cdc_single #(.DEST_SYNC_FF(2),.INIT_SYNC_FF(1),.SRC_INPUT_REG(0),.SIM_ASSERT_CHK(1))
 ...
 always_ff @(posedge clk_fast or posedge rst_fast)begin
  if(rst_fast)begin mailbox_valid<=0;mailbox_run<=0;end
   ...
   if(fast_reset_busy)mailbox_valid<=0;
    ...
    if(write_fire)begin mailbox_data<=fifo_in;mailbox_valid<=1;end
   end
  end
 end
 wire read_fire;
 ...
 always_ff @(posedge clk_fast or posedge rst_fast)begin
  ...
  else if(fast_reset_busy)begin closed_fast<=0;expected_window<=0;done_seen<=done_sync;end
  else begin
   done_seen<=done_sync;
 ...
 always_ff @(posedge clk_slow or posedge rst_slow)begin
  ...
  end else if(slow_reset_busy)begin
