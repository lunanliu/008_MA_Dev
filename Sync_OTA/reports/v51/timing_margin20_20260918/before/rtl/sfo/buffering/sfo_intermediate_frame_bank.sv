...
module sfo_intermediate_frame_bank #(
  ...
  wire completely_free=!source_valid&&!filled&&!committed&&!reading&&!qvalid&&!qbusy&&rv==0&&!ram_we&&!ram_re&&!close_pending;
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (rv[2] && qready) returned <= returned + 1;
      if (!halted && bad == 0) begin
        ...
        if(ram_we)begin
          ...
          source_written_exclusive<=19'(ram_wa+1);
          if(ram_last)begin filled<=1;write_done<=1;close_pending<=0;end
        end
        if (wf) begin
          if (!writing) begin
            write_frame <= s_frame;
