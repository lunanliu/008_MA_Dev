...
module cfo_divide_rne64wide(
 input logic clk,rst,abort_sync,input logic s_valid,output logic s_ready,
 input logic signed [63:0] s_numerator,input logic [63:0] s_denominator,
 output logic m_valid,input logic m_ready,output logic signed [63:0] m_quotient,output logic m_error
);
 typedef enum logic [2:0] {IDLE,DIVIDE,ROUND_PRE,ROUND,RESULT} state_t;
 state_t state;logic negative;logic [63:0] numerator,quotient;
 logic [63:0] denominator;logic [64:0] remainder;logic [5:0] count;
 wire clear=rst||abort_sync;
 assign s_ready=!clear && state==IDLE;assign m_valid=!clear && state==RESULT;
 wire [64:0] shifted={remainder[63:0],numerator[63]};
 wire ge=shifted>={1'b0,denominator};
 wire [64:0] next_rem=ge ? shifted-{1'b0,denominator}:shifted;
 wire [64:0] doubled=remainder<<1;
 wire inc=(doubled>{1'b0,denominator})||((doubled=={1'b0,denominator})&&quotient[0]);
 logic [63:0] rounded;
 always_ff @(posedge clk)begin
  if(clear)begin state<=IDLE;negative<=0;numerator<=0;quotient<=0;denominator<=0;remainder<=0;count<=0;rounded<=0;m_quotient<=0;m_error<=0;end
  else case(state)
   ...
   DIVIDE:begin
    ...
    if(count==63)state<=ROUND_PRE;else count<=count+1'b1;
   end
   ROUND_PRE:begin rounded<=quotient+{{63{1'b0}},inc};state<=ROUND;end
