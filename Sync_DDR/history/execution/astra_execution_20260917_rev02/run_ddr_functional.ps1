...
$out=Join-Path $root "validation_native_02"
...
try{
try{
 Register-CimIndicationEvent -Namespace root/cimv2 -Query "SELECT * FROM Win32_ProcessStartTrace" -SourceIdentifier $script:eventId -ErrorAction Stop|Out-Null
 $registered=$true
}catch{
 $registered=$false
 ("WMI process-start event unavailable; polling fallback: " + $_.Exception.Message)|Add-Content -LiteralPath (Join-Path $out "warnings.log")
}
 $registered=$true
