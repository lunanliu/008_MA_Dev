...
$out=Join-Path $root 'validation_native_03'
...
for($i=0;$i-lt 3;$i++){
 ...
 $actualArgs=@($argsExpected);$actualArgs[5]=Join-Path $out ($name+'.log');$actualArgs[7]=Join-Path $out ($name+'.jou')
...
Need ($FirstStartNotAfterUtc-ne[datetime]::MinValue -and [datetime]::UtcNow-lt$FirstStartNotAfterUtc.ToUniversalTime()) 'First-start window absent or expired.'
New-Item -ItemType Directory -Path $out|Out-Null
JsonFile $preflight (Join-Path $out 'preflight.json')
$summary=[ordered]@{job=$job.job_id;grant_id=$GrantId;status='RUNNING';started_utc=[datetime]::UtcNow.ToString('o');native_deadline_utc=$null;stages=@();error=$null;cleanup=$null;finished_utc=$null}
$deadline=$null;$current=$null;$guard=$null;$lastMemory=[datetime]::MinValue
