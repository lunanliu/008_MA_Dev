...
$out=Join-Path $root 'validation_native_05'
...
Need (-not(Test-Path -LiteralPath (Join-Path $root 'project_retry05'))) 'Project already exists; preserve prior evidence.'
Need (-not(Test-Path -LiteralPath (Join-Path $root 'actual_sources_retry05.txt'))) 'Source export already exists; preserve evidence.'
...
function CheckActualSources{
 ...
 $actual=@(Get-Content -LiteralPath (Join-Path $root 'actual_sources_retry05.txt')|ForEach-Object{
...
function CheckNativeLogs($stage){
 ...
 if($stage.name-ne'create'){
  $simDir=Join-Path $root ('project_retry05\DDR_Control.sim\sim_'+$stage.name+'\behav\xsim')
...
$preflight.actual_project=Join-Path $root 'project_retry05\DDR_Control.xpr'
...
$retry04=Get-Content -LiteralPath (Join-Path $root 'validation_native_04\summary.json') -Raw|ConvertFrom-Json
Need ($retry04.cleanup.closed -and @($retry04.stages).Count-eq 1 -and $retry04.stages[0].stage-eq'create' -and $retry04.status-eq'FAIL') 'Retry04 not closed or already advanced.'
