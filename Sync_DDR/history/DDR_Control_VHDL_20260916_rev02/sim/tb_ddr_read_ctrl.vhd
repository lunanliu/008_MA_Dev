...
entity tb_ddr_read_ctrl is
end entity;

architecture test of tb_ddr_read_ctrl is
  subtype word_t is std_logic_vector(31 downto 0);
  type block_t is array (0 to 39) of word_t;
  ...
  begin
    ...
    index := effective_address * 40 + lane;
...
begin
  dut : entity work.ddr_read_ctrl
    port map (
      ...
      ddr_word_count => ddr_word_count, pattern_seed => pattern_seed,
  ...
  begin
    wait for 900 us;
    assert finished report "DDR read TB exceeded 900 us simulation budget" severity failure;
    wait;
  end process;
  ...
  begin
    ...
    if offset > 36 then offset := 0; end if;
    ...
    if falling_edge(clk) then
      ...
      if reset = '1' or model_clear = '1' then
        ...
        if stream_fire = '1' then
          ...
          assert as_natural(stream_offset) <= 36 and as_natural(stream_offset) mod 4 = 0
          ...
          for j in 0 to 3 loop
            ...
            expected := memory_word((sent + j) / 40, (sent + j) mod 40,
              model_sample_count, model_seed, corrupt_index, swap_blocks);
        ...
        if ddr_retrieve_valid = '1' then
          ...
          if not swap_blocks then
            for j in 0 to 39 loop
              if address * 40 + j >= model_sample_count then
        ...
        if request_valid = '1' then
          ...
          assert requests < (model_sample_count + 39) / 40
  ...
  stimulus : process
    ...
    begin
      ...
      ddr_word_count <= std_logic_vector(to_unsigned((n + 39) / 40, 32));
    ...
    begin
      ...
      if not swap_blocks then
        assert model_tail_words = m * 40 - n
  ...
  begin
    ...
    pattern_seed <= x"DEADBEEF"; pattern_check_enable <= '0';
    ...
    g := as_natural(read_generation); read_command <= '1'; tick;
