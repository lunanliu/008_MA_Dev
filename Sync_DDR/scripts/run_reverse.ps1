...
[CmdletBinding()] param([switch]$ValidateOnly,[ValidateSet('create','capture','download')][string]$Stage='create',[string]$GrantFile)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
$root=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$request=Get-Content -LiteralPath (Join-Path $root 'reports/operations/RESOURCE_REQUEST_FOUR_STAGE_V1.json') -Raw | ConvertFrom-Json
...
Need ($grant.grant_id -notmatch 'DDR_REV02_REV03|RUNTIME_CONFIG') 'Old grant forbidden'
...
if($Stage-eq'download'){Need ($state.passed -contains 'capture') 'Capture must pass first'}
...
$source=Join-Path $PSScriptRoot $(if($Stage-eq'create'){'create_four_stage_project.tcl'}else{'simulate_reverse.tcl'})
...
try{
 ...
 if($Stage-eq'create'){
  CheckSourceSet
  Need ((Get-Content -LiteralPath (Join-Path $out 'vivado.log') -Raw).Contains('DDR_FOUR_STAGE_PROJECT_CREATED')) 'Missing create marker'
  ...
  $marker=if($Stage-eq'capture'){'DDR_CAPTURE_CTRL_TEST_PASS'}else{'DDR_DOWNLOAD_CTRL_TEST_PASS'}
