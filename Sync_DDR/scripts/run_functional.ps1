...
[CmdletBinding()] param([switch]$ValidateOnly,[ValidateSet('create','write','read')][string]$Stage='create',[string]$GrantFile)
Set-StrictMode -Version Latest
$ErrorActionPreference='Stop'
$root=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '..'))
$request=Get-Content -LiteralPath (Join-Path $root 'reports/operations/RESOURCE_REQUEST.json') -Raw | ConvertFrom-Json
...
Need ($grant.grant_id -notmatch 'DDR_REV02_REV03') 'Old grant forbidden'
...
if($Stage-eq'read'){Need ($state.passed -contains 'write') 'Write must pass first'}
...
$source=Join-Path $PSScriptRoot $(if($Stage-eq'create'){'create_project.tcl'}else{'simulate.tcl'})
...
function CheckSourceSet{
 $tab=[char]9
 $expected=@(
 ('sources_1'+$tab+(Canon (Join-Path $root 'rtl/ddr_upload_ctrl.vhd')))
 ('sources_1'+$tab+(Canon (Join-Path $root 'rtl/ddr_read_ctrl.vhd')))
 ('sim_write'+$tab+(Canon (Join-Path $root 'sim/tb_ddr_upload_ctrl.vhd')))
 ('sim_read'+$tab+(Canon (Join-Path $root 'sim/tb_ddr_read_ctrl.vhd')))
 ('constrs_1'+$tab+(Canon (Join-Path $root 'constraints/upload_150mhz.xdc')))
 )
 ...
 Need ($actual.Count-eq 5 -and @(Compare-Object ($expected|Sort-Object) ($actual|Sort-Object)).Count-eq 0) 'Source set mismatch'
 $tops=@(Get-Content -LiteralPath (Join-Path $out 'actual_tops.txt'))
 $want=@(('sources_1'+$tab+'ddr_upload_ctrl'),('sim_write'+$tab+'tb_ddr_upload_ctrl'),('sim_read'+$tab+'tb_ddr_read_ctrl'))
...
try{
 ...
 if($Stage-eq'create'){
  CheckSourceSet
  Need ((Get-Content -LiteralPath (Join-Path $out 'vivado.log') -Raw).Contains('DDR_CONTROL_PROJECT_CREATED')) 'Missing create marker'
  ...
  $marker=if($Stage-eq'write'){'DDR_UPLOAD_CTRL_TEST_PASS'}else{'DDR_READ_CTRL_TEST_PASS'}
 ...
 if($Stage-ne'create' -and $null-ne$state.project){
  ...
  if(Test-Path -LiteralPath $dir){
   ...
   Get-ChildItem -LiteralPath $dir -File | Where-Object {$_.Extension -in '.log','.prj','.tcl','.bat'} | ForEach-Object {Copy-Item -LiteralPath $_.FullName -Destination $saved}
