...
foreach {mode tb} {write tb_ddr_upload_ctrl read tb_ddr_read_ctrl capture tb_ddr_capture_ctrl download tb_ddr_download_ctrl} {
  set fs sim_$mode
...
foreach fs {sources_1 sim_write sim_read sim_capture sim_download constrs_1 constrs_read_125} {
  foreach src [get_files -of_objects [get_filesets $fs]] {puts $f "$fs\t[file normalize $src]"}
}
close $f
set f [open [file join $out actual_tops.txt] w]
foreach fs {sources_1 sim_write sim_read sim_capture sim_download} {puts $f "$fs\t[get_property top [get_filesets $fs]]"}
close $f
puts "DDR_FOUR_STAGE_PROJECT_CREATED"
close_project
