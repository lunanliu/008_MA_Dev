...
foreach name {ddr_upload_ctrl ddr_read_ctrl ddr_capture_ctrl ddr_download_ctrl} {
  add_files -norecurse [list [file join $root rtl $name.vhd]]
  set_property file_type VHDL [get_files $name.vhd]
}
set_property top ddr_capture_ctrl [get_filesets sources_1]
add_files -fileset constrs_1 -norecurse [list [file join $root constraints controller_150mhz.xdc]]
create_fileset -constrset constrs_read_125
add_files -fileset constrs_read_125 -norecurse [list [file join $root constraints controller_125mhz.xdc]]
foreach {mode tb} {write tb_ddr_upload_ctrl read tb_ddr_read_ctrl capture tb_ddr_capture_ctrl download tb_ddr_download_ctrl roundtrip tb_ddr_roundtrip} {
  set fs sim_$mode
...
foreach fs {sources_1 sim_write sim_read sim_capture sim_download sim_roundtrip constrs_1 constrs_read_125} {
  foreach src [get_files -of_objects [get_filesets $fs]] {puts $f "$fs\t[file normalize $src]"}
}
close $f
set f [open [file join $out actual_tops.txt] w]
foreach fs {sources_1 sim_write sim_read sim_capture sim_download sim_roundtrip} {puts $f "$fs\t[get_property top [get_filesets $fs]]"}
close $f
puts "DDR_ROUNDTRIP_PROJECT_CREATED"
close_project
