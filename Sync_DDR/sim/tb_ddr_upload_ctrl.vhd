...
begin
  ...
  begin
    tick(3); reset<='0'; tick;
    if SMOKE_ONLY then
      begin_upload(K+4,2,1000); await_done(K+4,2);
      begin_upload(4,1,2000); await_done(4,1);
      report "UPLOAD_ALT_WIDTH_CASE_PASS" severity note;
      finished<=true; wait;
    end if;
    invalid_params(0,1); invalid_params(6,1); invalid_params(44,1);
    invalid_params(44,3);
    ddr_capacity_words<=x"00000800"; tick; invalid_params(81924,2049);
    ddr_capacity_words<=(others=>'0'); tick; invalid_params(4,1);
    ...
    sample_count<=x"00000006"; ddr_word_count<=x"00000000"; ddr_capacity_words<=(others=>'0');
    tick(7);
    ...
    -- Capture above bit 31: unsigned snapshot must not become signed integer.
    low_commands; source_on<='0';
    sample_count<=x"FFFFFFFC"; ddr_word_count<=x"06666667";
    ddr_capacity_words<=x"FFFFFFFF"; tick; load_command<='1'; tick(3);
    assert load_busy='1' and load_fault='0' report "U32 range rejected" severity failure;
    abort_command<='1'; tick; abort_command<='0'; rearm;
    source_on<='1'; ddr_capacity_words<=x"00020000"; stall_pattern<=false; tick;
    begin_upload(1336860,33422,200000); await_done(1336860,33422);
    report "UPLOAD_FULL_FRAME_1336860_PASS" severity note;
    report "UPLOAD_DEFAULT_CASE_PASS" severity note;
    finished<=true;
    wait;
  end process;
end architecture;
