...
module cfo_front2048_window(
 ...
 logic [16:0] coefficient_base,coefficient_address;
 ...
 always_comb begin
  ...
  coefficient_address=coefficient_base+{7'd0,selected_index};
 ...
 always_ff @(posedge clk)begin
  if(selected)begin coefficient_index0<=coefficient_index_rom[coefficient_address];f_pipe[0]<={f_i,f_q};end
 ...
 always_ff @(posedge clk)begin
  ...
  if(rst || abort_sync)begin
   state<=FEED;first_pending<=1;coefficient_base<=0;v<=0;fft_done<=0;sum_done<=0;sum_i<=0;sum_q<=0;pilot_count<=0;
   ...
   v<={v[4:0],selected};
   if(selected)index_pipe[0]<=selected_index;
   ...
   if(fft_m_valid && fft_m_ready && f_error!=0)begin
    state<=HOLD;m_error<=f_error;m_z_i<=0;m_z_q<=0;m_pilot_count<=0;m_fft_saturations<=0;v<=0;
   end
   if(state==HOLD && m_ready)begin
    state<=FEED;first_pending<=1;v<=0;fft_done<=0;sum_done<=0;sum_i<=0;sum_q<=0;pilot_count<=0;
   end
  end
 end
endmodule