...
import json,hashlib,csv,argparse,importlib.util
def helper(name):
 s=importlib.util.spec_from_file_location(name,R/'tools'/f'{name}.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m);return m
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--sources',type=Path);ap.add_argument('--identity',type=Path);ap.add_argument('--actual',type=Path);ap.add_argument('--reset-audit',type=Path);a=ap.parse_args()
 lock=load(R/'docs/LINK010R1_SOURCE_LOCK.json')
 for x in lock['files']+lock['vendor_dependencies']:
  p=R/x['path'];assert p.stat().st_size==x['bytes'] and sha(p)==x['sha256'],str(p)
 model_check=helper('verify_link010r1_models').run()
 ...
 out={'status':'LINK010R1_SOURCE_VECTOR_PASS','frozen_files':len(lock['files']),'vendor_dependencies':len(lock['vendor_dependencies']),'cases':4,'real_front009_windows':74,'protocol_error_vectors':10,'native_checked':False,'no_fft_or_estimator_recomputation':True}
 if a.sources:
  ...
  # Used-in/library properties prove the two alternative XPM source sets remain separate.
  bypath={str(Path(x['path']).resolve()).casefold():x for x in rows}
  for name in ['xpm_cdc','xpm_memory','xpm_fifo']:
   official=bypath[str(Path(f'C:/NIFPGA/programs/Vivado2021_1/data/ip/xpm/{name}/hdl/{name}.sv').resolve()).casefold()]
   private=bypath[str((R/f'sim/vendor/link010r1/{name}.sv').resolve()).casefold()]
   def flag(x,k):
    v=x[k].lower();assert v in ['1','0','true','false'],(k,v);return v in ['1','true']
   assert flag(official,'used_in_synthesis') and not flag(official,'used_in_simulation'),official
   assert private['library']=='cfo_xpm' and flag(private,'used_in_simulation') and not flag(private,'used_in_synthesis') and not flag(private,'used_in_implementation'),private
  out['explicit_private_vendor_sources']='PASS'
 if a.identity:
  p=dict(x.split('=',1) for x in a.identity.read_text('utf-8-sig').splitlines())
  for k,v in {'part':'xcvu11p-flgb2104-2-e','hardware_top':'cfo_estimator_link','simulation_top':'cfo_estimator_link_tb','vivado':'2021.1','xelab_jobs':'16','xpm_libraries':''}.items():assert p[k]==v,(k,p[k])
  assert Path(p['project']).resolve()==(R/'vivado/CFO_LINK010R1/CFO_LINK010R1.xpr').resolve();out['project_identity']='PASS'
 if a.actual:
  ...
  out.update(status='LINK010R1_NUMERIC_RESET_PASS_LIMITED_SCOPE',native_checked=True,rows=counts,max_tail_slow_cycles=max_tail,max_total_fast_cycles=max_total,source_backpressure_cycles=stalls,burst_full_runs=full_runs,complete_frames=22,discarded_frames=8,protocol_errors=10,modes=[1,2,0,3],whole_frame_waveform_tested=False,physical_cdc_qualified=False,synthesis_or_timing_qualified=False,formal_T11_T12_T13_PASS=False)
 out['private_model_check']=model_check
 if a.actual:
  assert a.reset_audit,'Required independent reset-edge audit is missing'
  out['reset_audit']=helper('verify_link010r1_reset').run(a.reset_audit)
