...
# FRONTDATA_CHUNK_2_END
    ...
    def raw_front(self, dset: int, shape: list[int], meta: dict[str, Any],
        ...
        for col in range(0, 820, cwidth):
            ...
            buffer_bytes = full_count * meta["size_bytes"]
            ...
            if mask.value & 1:
                ...
                stored = buffer_bytes
                decoded = raw
                decode_method = "raw_chunk_filter_mask_bit0_set"
            else:
                stream = zlib.decompressobj()
                decoded = stream.decompress(raw_buffer) + stream.flush()
                if not stream.eof:
                    raise ValueError(f"{label}: zlib stream did not reach EOF at {col}")
                used = buffer_bytes - len(stream.unused_data)
                if any(stream.unused_data):
                    raise ValueError(f"{label}: nonzero bytes after zlib stream at {col}")
                raw = raw_buffer[:used]
                stored = used
                decode_method = "zlib_deflate"
            ...
            chunk_records.append({
                ...
                "read_buffer_bytes": buffer_bytes, "decode_method": decode_method,
