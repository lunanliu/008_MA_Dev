#!/usr/bin/env python3
"""CFO-GUARD002: private Windows Job-object supervisor for frozen native probes."""
...
JOB_EXT=9; JOB_PIDS=3; SNAP=2; QLIMIT=0x1000; VMREAD=0x10; SYNC=0x100000; STILL=259; INHERIT=1
...
k.CreateJobObjectW.restype=w.HANDLE; k.SetInformationJobObject.restype=w.BOOL; k.QueryInformationJobObject.restype=w.BOOL; k.AssignProcessToJobObject.restype=w.BOOL; k.TerminateJobObject.restype=w.BOOL
k.CreateProcessW.restype=w.BOOL; k.ResumeThread.restype=w.DWORD; k.TerminateProcess.restype=w.BOOL; k.GetExitCodeProcess.restype=w.BOOL; k.OpenProcess.restype=w.HANDLE; k.SetHandleInformation.restype=w.BOOL; k.GlobalMemoryStatusEx.restype=w.BOOL; k.GetProcessTimes.restype=w.BOOL; k.QueryFullProcessImageNameW.restype=w.BOOL; k.CreateToolhelp32Snapshot.restype=w.HANDLE; k.Process32FirstW.restype=w.BOOL; k.Process32NextW.restype=w.BOOL; p.GetProcessMemoryInfo.restype=w.BOOL
...
def detail(pid,ent=None):
    ...
    return d
def snapshot(pids): 
    t=table(); return [detail(x,t.get(x)) for x in sorted(set(pids))]
def native_snapshot():
    ...
    def pids(self):
        ...
        for _ in range(6):
            ...
            if k.QueryInformationJobObject(self.h,JOB_PIDS,b,size,ctypes.byref(ret)):
                n=int.from_bytes(b.raw[4:8],"little"); return [int.from_bytes(b.raw[8+4*i:12+4*i],"little") for i in range(n)],None
            if ctypes.get_last_error()==122: size*=2; continue
            return [],err("QueryInformationJobObject(PID list)")
        return [],"PID-list buffer growth exhausted"
    def extended(self):
        x=JEXT(); ret=w.DWORD()
    ...
    def run(self,mode):
        ...
        try:
            ...
            while True:
                ...
                pids,qerr=job.pids();tree=snapshot(pids);mm=mem();ji=job.extended();mon=detail(os.getpid(),table().get(os.getpid()))
                sm={"utc":utc(),"monotonic_ns":now,"mode":mode,"active_process_count":len(pids),"active_process_ids":pids,"active_process_tree":tree,"job_active_query_error":qerr,"system_memory":mm,"instant_job_private_commit_bytes":(0 if not pids else sum(int(x["private_usage_bytes"]) for x in tree) if len(tree)==len(pids) and all(x.get("private_usage_bytes") is not None for x in tree) else None),"job_memory":ji,"monitor_private_usage_bytes":mon.get("private_usage_bytes"),"stage_progress":mk,"root_exit_code":proc.exit()}
                ...
                if triggered and not pids and qerr is None:R["events"]["zero"]=utc();R["event_mono_ns"]["zero"]=now;self.log.event("job_zero",mode=mode);break
                ...
                if not triggered and root_exit is not None and not pids and qerr is None:R["events"]["zero"]=utc();R["event_mono_ns"]["zero"]=now;self.log.event("job_zero_natural",mode=mode);break
                time.sleep(.1)
            R["markers"]=markers(paths,mode);R["native_exit_code"]=proc.exit();R["active_pids_at_loop_end"],R["active_query_error_at_loop_end"]=job.pids();R["job_extended_at_loop_end"]=job.extended()
            ...
            if job is not None:
                try:
                    left,q=job.pids()
                    if left and not R.get("termination_requested_by_controller"):job.kill();R["termination_requested_by_controller"]=True;R["termination_reason"]=R.get("termination_reason") or "FINALLY_JOB_CLEANUP"
                    deadline=mono()+5_000_000_000
                    while left and mono()<deadline:time.sleep(.1);left,q=job.pids()
                    R["active_pids_after_finally"]=left;R["active_query_error_after_finally"]=q
...
def main():
    ...
    if a.self_check:
        ...
        print(json.dumps({"schema":"cfo_guard002_self_check_v1","checks":checks}));return 0 if all(checks.values()) else 2
    parent=root/"work/CFO_GUARD002";parent.mkdir(parents=True,exist_ok=True);attempt=parent/f"attempt_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}_luna";attempt.mkdir();write_json(attempt/"report_clock.json",{"schema":"cfo_guard002_report_clock_v1","thread_id":"01a076f0-d8c0-72a0-8371-cb2ff29c1b28","started_utc":utc(),"interval_minutes":15,"status":"ACTIVE"});log=Log(attempt/"guard_controller.log")
    try:
        ...
        if not all(frozen["checks"].values()):
            c={"schema":"cfo_guard002_completion_v1","status":"BLOCKED_PREFLIGHT","attempt":str(attempt),"execution_freeze_sha256":fh,"checks":frozen["checks"]};ch=write_json(attempt/"ATTEMPT_COMPLETION.json",c);print(json.dumps({"status":c["status"],"attempt":str(attempt),"completion_sha256":ch}));return 3
        ...
        c={"schema":"cfo_guard002_completion_v1","job_id":"CFO-GUARD002","generated_utc":utc(),"attempt":str(attempt),"controller_pid":os.getpid(),"execution_freeze":{"path":str(attempt/"EXECUTION_FREEZE.json"),"sha256":fh},"modes":results,"all_modes_pass":len(results)==3 and all(r.get("status")=="PASS" for r in results),"binding_failure_stopped_remaining":binding_stop,"post_native_processes":native_snapshot(),"t10_not_operated_by_controller":True,"no_matlab":True,"no_computation_rerun":True,"no_compile_simulation_synthesis_or_ip_generation":True,"scope":"Only frozen normal/error/hold guard_probe_v2.tcl execution protection."}
        ...
        log.event("controller_exception",error=repr(e));c={"schema":"cfo_guard002_completion_v1","status":"CONTROLLER_FAILURE","generated_utc":utc(),"attempt":str(attempt),"error":repr(e)}
