#!/usr/bin/env python3
"""CFO-FRONT009 native create/simulate supervisor for the frozen FRONT2048 arithmetic path."""
...
LOCK=ROOT/"docs/FRONT009_SOURCE_LOCK.json"; CONTRACT=ROOT/"docs/FRONT2048_CONTRACT_V1_ZH.md"; DOC=ROOT/"docs/FRONT2048_NATIVE_JOB_009.md"; VERIFY=ROOT/"tools/verify_front2048.py"
TCL=ROOT/"vivado/front2048_project.tcl"; THREADS=ROOT/"vivado/run_threads.tcl"; CONFIG=ROOT/"vivado/configure_parallel_jobs.tcl"; XPR=ROOT/"vivado/CFO_FRONT2048/CFO_FRONT2048.xpr"
FFT_PROJECT=ROOT/"vivado/CFO_FRONT2048"; FFT_SIM=FFT_PROJECT/"CFO_FRONT2048.sim/sim_1/behav/xsim"; REVIEW=ROOT/"reports/FFT008_REVIEW_20260914/INDEPENDENT_REVIEW.json"
LOCK_SHA="09A6A7F5AD994934B9CA30137139A7639A16638DB966AB3498AEDA9CF9765393"; HARD_CREATE=120; HARD_SIM=600; ZERO=5; FREE=6*1024**3
...
def fft_paths(root,attempt,stage):
    ps=[attempt/f"{stage}.log",attempt/f"{stage}.jou",attempt/f"{stage}.stdout.log",attempt/f"{stage}.stderr.log",attempt/f"{stage}_actual_sources.csv",attempt/f"{stage}_project_identity.txt",attempt/"front2048_actual.txt"]
    ps += [FFT_SIM/n for n in ("xvlog.log","compile.log","elaborate.log","xelab.log","simulate.log","xsim.log","xsim.dir/cfo_front2048_tb_behav/xsimkernel.log")]
    out=[];seen=set()
    for p in ps:
...
def fft_marks(root,attempt,stage):
    ...
    return {"ready":Q.marker(ps,f"CFO_FRONT2048_READY action={stage} "),"done":Q.marker(ps,f"CFO_FRONT2048_NATIVE_DONE action={stage}"),"passed":Q.marker(ps,"CFO_FRONT2048_PASS ")}
Q.paths=fft_paths
Q.marks=fft_marks
...
def command(attempt,stage):
    return [str(Q.VIVADO),"-mode","batch","-source","D:/008_MA_Dev/T11_CFO/vivado/front2048_project.tcl","-log",str(attempt/f"{stage}.log"),"-journal",str(attempt/f"{stage}.jou"),"-tclargs",stage,str(attempt)]
def freeze(root,attempt,pre,base,mem,lock,rows,lockok):
    old={str(p):info(p) for p in [Q.OLD_SYNC_XPR,Q.OLD_COORD_XPR,Q.PHASE_XPR,ROOT/"vivado/CFO_FFT2048/CFO_FFT2048.xpr",ROOT/"vivado/CFO_BACKEND74/CFO_BACKEND74.xpr"]}
...
def pass_ok(r,verify_result,source_check):
    ...
    good=bool(v and v["unique"]==90 and v["completed"]==98 and v["protocol_errors"]==7 and v["stalled_cycles"]==250 and v["reset_discards"]==4 and v["abort_discards"]==4 and v["max_tail"]<=13500 and v["max_total"]<=19000 and v["max_adjusted"]<=15600)
...
def complete(attempt,root,payload):
    ...
    mh=wjson(attempt/"ATTEMPT_ARTIFACT_MANIFEST.json",{"schema":"cfo_front009_artifact_manifest_v1","generated_utc":utc(),"attempt":str(attempt),"completion_sha256":ch,"artifacts":artifacts(attempt)})
    return ch,mh
def verify(attempt,stage,actual=False):
    av=[]
    if actual: av += ["--actual",attempt/"front2048_actual.txt"]
...
def main():
    ...
    if q.self_check:
        c={"root":root.is_dir(),"guard":GUARD.is_file(),"r1_wrapper":R1.is_file(),"v3_pointer_size":Q.G.PTR_SIZE==8,"v3_accounting_class":Q.G.JOB_ACCOUNTING==1,"fft_tcl":TCL.is_file(),"source_lock":LOCK.is_file(),"fft_verify":VERIFY.is_file(),"no_native_started":True};print(json.dumps({"schema":"cfo_front009_controller_self_check_v1","checks":c,"native_started":False}));return 0 if all(c.values()) else 2
    parent=root/"work/CFO_FRONT009";parent.mkdir(parents=True,exist_ok=True);attempt=parent/f"attempt_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}_luna";attempt.mkdir();ep=attempt/"controller_events.jsonl";shutil.copy2(Path(__file__).resolve(),attempt/"cfo_front2048_guard_v1_frozen.py");wjson(attempt/"report_clock.json",{"schema":"cfo_front009_report_clock_v1","thread_id":"01a076f0-d8c0-72a0-8371-cb2ff29c1b28","started_utc":utc(),"interval_minutes":15,"status":"ACTIVE"})
    ...
    try: shutil.copy2(XPR,attempt/"CFO_FRONT2048_after_create.xpr")
    except Exception: pass
    out["create"]["xpr_copy"]=info(attempt/"CFO_FRONT2048_after_create.xpr");out["create"]["status"]="PASS" if create_ok(out["create"],out["create"]["verify"],out["create"]["source_check"]) and out["create"]["xpr"].get("exists") else "FAIL";wjson(attempt/"create_verify.json",out["create"]["verify"]);wjson(attempt/"create_source_check.json",out["create"]["source_check"])
    if out["create"]["status"]!="PASS":
        c={"schema":"cfo_front009_completion_v1","job_id":"CFO-FRONT009","status":"BLOCKED_AFTER_CREATE","attempt":str(attempt),"generated_utc":utc(),"execution_freeze":{"path":str(attempt/"EXECUTION_FREEZE.json"),"sha256":fsha},"stages":out,"post_native_processes":Q.G.native_snapshot(),"scope":"Create failed or source/identity verification failed; simulate not started.","t10_not_operated_by_controller":True,"no_matlab":True,"repair_audit":{"old_project_rebuilt":False}};ch,mh=complete(attempt,root,c);print(json.dumps({"status":c["status"],"attempt":str(attempt),"completion_sha256":ch,"manifest_sha256":mh}));return 5
    ...
    if not pre_sim_ok:
        payload={'schema':'cfo_front009_completion_v1','status':'BLOCKED_BEFORE_SIMULATE','attempt':str(attempt),'stages':out,'successful_create_checkpoint_retained':True,'native_simulation_started':False}
    ...
    try: shutil.copy2(XPR,attempt/"CFO_FRONT2048_final.xpr")
    except Exception: pass
    out["simulate"]["xpr_copy"]=info(attempt/"CFO_FRONT2048_final.xpr");out["simulate"]["status"]="PASS" if pass_ok(out["simulate"],out["simulate"]["verify"],out["simulate"]["source_check"]) and out["simulate"]["xpr"].get("exists") else "FAIL";wjson(attempt/"simulate_verify.json",out["simulate"]["verify"]);wjson(attempt/"simulate_source_check.json",out["simulate"]["source_check"])
    xa={"schema":"cfo_front009_xpr_audit_v1","after_create":out["create"]["xpr"],"final":out["simulate"]["xpr"],"after_create_copy":out["create"]["xpr_copy"],"final_copy":out["simulate"]["xpr_copy"]};xsha=wjson(attempt/"FINAL_XPR_AUDIT.json",xa);post=Q.G.native_snapshot();tgood,tdelta=t10_ok(base,{"all_native_processes":post});post_lock,post_rows,post_ok=locked(root,LOCK);controller_unchanged=info(Path(__file__).resolve()).get("sha256")==info(attempt/"cfo_front2048_guard_v1_frozen.py").get("sha256");old_unchanged=all(info(Path(p)).get("sha256")==x.get("sha256") for p,x in fr["frozen_project"]["old_project_fingerprints"].items());status="PASS" if all(out[k]["status"]=="PASS" for k in ("create","simulate")) and tgood and post_ok and controller_unchanged and old_unchanged else "FAIL"
    c={"schema":"cfo_front009_completion_v1","job_id":"CFO-FRONT009","status":status,"attempt":str(attempt),"generated_utc":utc(),"execution_freeze":{"path":str(attempt/"EXECUTION_FREEZE.json"),"sha256":fsha},"stages":out,"final_xpr_audit":{"path":str(attempt/"FINAL_XPR_AUDIT.json"),"sha256":xsha,**xa},"post_source_lock":{"lock":post_lock,"members":post_rows,"all_members_match":post_ok},"post_native_processes":post,"t10_unchanged":tdelta,"controller_bytes_unchanged":controller_unchanged,"old_project_bytes_unchanged":old_unchanged,"t10_not_operated_by_controller":True,"no_matlab":True,"no_old_runner":True,"no_old_stage_repeat":True,"no_synthesis_or_implementation":True,"scope":"Only the integrated window FFT2048/pilot multiply/sum create and behavioral XSim stages ran serially, including all 74 coefficient rows for one source frame. No contiguous full-frame waveform, estimator backend, CDC, T11/T12/T13, synthesis, timing, or sustained-throughput claim.","repair_audit":{"rtl_or_vector_modified_by_wrapper":False,"old_project_rebuilt":False}};ch,mh=complete(attempt,root,c);print(json.dumps({"status":status,"attempt":str(attempt),"completion_sha256":ch,"manifest_sha256":mh}));return 0 if status=="PASS" else 6
