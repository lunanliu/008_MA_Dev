"""Read-only GUI/process admission repair for the reviewed LINK010R1 compat1 v2 entrypoint."""
...
RUNNER_PATH = ROOT / "tools/cfo_link010r1_compat1_guard_v2_repair.py"
...
def snapshot(stage, *, Q=None, gui_log_paths=None, slot_path=BASE.A.SLOT):
    if stage != "simulate":
        raise ValueError("compat1 v2 admission is simulate-only")
    ...
    result["compat1_v2_exact_grant_checks"] = exact
    result["compat1_v2_dependency_checks"] = deps
    result["all_checks"] = bool(result.get("all_checks")) and all(exact.values())
    result["blocking_checks"] = list(result.get("blocking_checks", []))
    result["blocking_checks"] += ["compat1_v2_exact_grant_" + key for key, value in exact.items() if not value]
    return result
...
def self_check():
    ...
    return {"schema": "link010r1_compat1_admission_v2_self_check_v1", "checks": checks,
