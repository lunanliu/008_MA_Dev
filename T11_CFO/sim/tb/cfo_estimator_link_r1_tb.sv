...
module cfo_estimator_link_tb;
 ...
 clocking reset_audit_fast_cb @(posedge clk_fast);
  default input #1step;
  input bits=reset_audit_bits;
  input epoch=reset_audit_epoch;
 endclocking
 ...
 always @(reset_audit_fast_cb)begin : reset_audit_fast_monitor
  ...
  bit changed,capture;
  if(reset_audit_enabled)begin
   pre=reset_audit_fast_cb.bits;pre_epoch=reset_audit_fast_cb.epoch;
 ...
 always @(reset_audit_slow_cb)begin : reset_audit_slow_monitor
  ...
  bit changed,capture;
  if(reset_audit_enabled)begin
   pre=reset_audit_slow_cb.bits;pre_epoch=reset_audit_slow_cb.epoch;
   edge_ps=reset_audit_ps();
