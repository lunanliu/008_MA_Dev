...
def create_ok(r,verify_result,source_check):
    ...
    return bool(r.get("binding_succeeded") and r.get("events",{}).get("zero") and f.get("empty_verified") and r.get("native_exit_code_final")==0 and not r.get("controller_errors") and not r.get("persistent_telemetry_fault") and not r.get("termination_requested_by_controller") and m.get("ready",{}).get("found") and m.get("done",{}).get("found") and r.get("ready_pid",0)>0 and r.get("stable_telemetry_samples",0)>=1 and r.get("unstable_after_ready_samples",0)<=3 and source_check.get("all_checks") and verify_result.get("exit_code")==0)
