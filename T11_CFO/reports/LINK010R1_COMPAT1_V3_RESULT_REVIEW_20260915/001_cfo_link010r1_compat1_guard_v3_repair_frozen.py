"""LINK010R1 compat1 v3 simulate-only controller with closed dependency/evidence gates."""
...
HELPER = ROOT / "tools/link010r1_compat1_admission_repair_v3.py"
HELPER_SHA = "8EF422428352F9EF991E59E6304F0EDF644C5CB7A7AA19B8FDE3EA99CC097899"
...
def add_required_prelaunch_error(errors, required, operation, detail):
    row = {"operation": operation, "detail": detail}
    errors.append(row)
    required.append(row)
...
def artifact_manifest(attempt, completion_sha, errors):
    ...
    return safe_wjson(attempt / "ATTEMPT_ARTIFACT_MANIFEST.json", {"schema": "link010r1_compat1_artifacts_v3",
...
def self_check():
    ...
    return {"schema": "link010r1_compat1_guard_v3_self_check_v1", "checks": checks, "all_checks": all(checks.values()),
...
def main():
    ...
    parent = ROOT / "work/CFO_LINK010R1_COMPAT1_V3"
    parent.mkdir(parents=True, exist_ok=True)
    attempt = parent / f"attempt_{dt.datetime.now(dt.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')}_luna"
    attempt.mkdir()
    errors = []
    required_prelaunch_errors = []
    frozen_runner = attempt / "cfo_link010r1_compat1_guard_v3_repair_frozen.py"
    frozen_runner_ok = False
    try:
        source_hash = file_sha(Path(__file__))
        shutil.copy2(Path(__file__), frozen_runner)
        frozen_runner_ok = frozen_runner.is_file() and file_sha(frozen_runner) == source_hash
        if not frozen_runner_ok:
            add_required_prelaunch_error(errors, required_prelaunch_errors, "freeze_runner",
                                          "frozen runner is missing or SHA does not match the live v3 runner")
    except Exception as exc:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "freeze_runner", repr(exc))
    ...
    for detail in freeze_errors:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "freeze_dependency", detail)
    preexisting_snapshot_ok = True
    try:
        preexisting_records = snapshot_preexisting_simdir(attempt)
        for row in preexisting_records:
            if row.get("exists") and row.get("copied") is not True:
                preexisting_snapshot_ok = False
                add_required_prelaunch_error(errors, required_prelaunch_errors, "preexisting_simdir_snapshot",
                                              row)
    except Exception as exc:
        preexisting_snapshot_ok = False
        add_required_prelaunch_error(errors, required_prelaunch_errors, "preexisting_simdir_snapshot", repr(exc))
    xpr_before = Q.info(BASE_XPR)
    pre_xpr_path = attempt / "CFO_LINK010R1_pre_compat1.xpr"
    pre_xpr_copy_ok = False
    try:
        shutil.copy2(BASE_XPR, pre_xpr_path)
        pre_xpr_copy_ok = pre_xpr_path.is_file() and file_sha(pre_xpr_path) == BASE_XPR_SHA
        if not pre_xpr_copy_ok:
            add_required_prelaunch_error(errors, required_prelaunch_errors, "copy_pre_xpr",
                                          "pre-simulation XPR copy is missing or SHA does not match the frozen XPR")
    except Exception as exc:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "copy_pre_xpr", repr(exc))
    ...
    checks = {"admission": admission.get("all_checks") is True, "source_lock": lock_result["all_checks"],
              ...
              "frozen_runner": frozen_runner_ok, "preexisting_simdir_snapshot": preexisting_snapshot_ok,
              "pre_xpr_copy": pre_xpr_copy_ok, "required_prelaunch_evidence": not required_prelaunch_errors}
    preflight = {"schema": "link010r1_compat1_pre_simulate_admission_v3", "admission": admission,
                 "source_check": lock_result, "dependency_check": deps_before, "frozen_dependencies": frozen_deps,
                 "reused_xpr": xpr_before, "static_checker": static, "checks": checks,
                 "required_prelaunch_evidence_errors": required_prelaunch_errors,
                 "all_checks": all(checks.values())}
    preflight_sha = safe_wjson(attempt / "PRE_SIMULATE_ADMISSION.json", preflight, errors)
    if not preflight_sha:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "write_pre_simulate_admission",
                                      "PRE_SIMULATE_ADMISSION.json write returned no SHA")
    freeze = {"schema": "link010r1_compat1_execution_freeze_v3", "job_id": AUTHORIZED_JOB, "utc": Q.utc(),
               ...
               "pre_simulate_admission_sha256": preflight_sha,
               "required_prelaunch_evidence_errors": required_prelaunch_errors, "checks": checks}
    freeze_sha = safe_wjson(attempt / "EXECUTION_FREEZE.json", freeze, errors)
    if not freeze_sha:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "write_execution_freeze",
                                      "EXECUTION_FREEZE.json write returned no SHA")
    checks["pre_simulate_admission_written"] = bool(preflight_sha)
    checks["execution_freeze_written"] = bool(freeze_sha)
    checks["required_prelaunch_evidence"] = not required_prelaunch_errors
    Q.event(events, "execution_freeze_written", sha256=freeze_sha, checks=checks)
    completion = {"schema": "link010r1_compat1_completion_v3", "attempt": str(attempt),
                  "pre_simulate_admission_sha256": preflight_sha, "execution_freeze_sha256": freeze_sha,
                  ...
                  "required_prelaunch_evidence_errors": required_prelaunch_errors, "evidence_errors": errors}
    ...
    launch_preflight_sha = safe_wjson(attempt / "PRE_SIMULATE_ADMISSION.json", preflight, errors)
    if not launch_preflight_sha:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "write_launch_pre_simulate_admission",
                                      "launch PRE_SIMULATE_ADMISSION.json write returned no SHA")
    freeze["launch_recheck"] = preflight["launch_recheck"]
    freeze["launch_pre_simulate_admission_sha256"] = launch_preflight_sha
    freeze["required_prelaunch_evidence_errors"] = required_prelaunch_errors
    freeze_sha = safe_wjson(attempt / "EXECUTION_FREEZE.json", freeze, errors)
    if not freeze_sha:
        add_required_prelaunch_error(errors, required_prelaunch_errors, "write_launch_execution_freeze",
                                      "launch EXECUTION_FREEZE.json write returned no SHA")
    launch_checks["pre_simulate_admission_written"] = bool(launch_preflight_sha)
    launch_checks["execution_freeze_written"] = bool(freeze_sha)
    launch_checks["required_prelaunch_evidence"] = not required_prelaunch_errors
    completion["execution_freeze_sha256"] = freeze_sha
    completion["launch_pre_simulate_admission_sha256"] = launch_preflight_sha
    completion["required_prelaunch_evidence_errors"] = required_prelaunch_errors
    Q.event(events, "execution_freeze_launch_recheck_written", sha256=freeze_sha, checks=launch_checks)
    if not all(launch_checks.values()) or required_prelaunch_errors or not launch_preflight_sha or not freeze_sha:
