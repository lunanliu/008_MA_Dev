...
def args_for() -> argparse.Namespace:
    ...
    parser.add_argument("--root-exit-grace-seconds", type=float, default=5.0)
...
def main() -> int:
    ...
    root_exit_mono = None
    post_root_cleanup_automatic = False
    root_rc = None
        ...
        while True:
            ...
            if root_exit_seen is not None:
                ...
                if remaining == 0:
                    ...
                    snap("job_empty", 0)
                    break
                if time.monotonic() - root_exit_mono >= max(0.0, args.root_exit_grace_seconds):
                    cancelled = True
                    post_root_cleanup_automatic = True
                    stop_note = "Root exited; automatic cleanup of this owned Job after the frozen grace interval"
                    snap("post_root_cleanup_automatic", remaining)
                    stop_owned("post_root_cleanup", 125)
                    break
                time.sleep(0.05)
    ...
    record = {
        ...
        "post_root_cleanup_automatic": post_root_cleanup_automatic,
        "root_exit_grace_seconds": args.root_exit_grace_seconds,
