...
打开vivado/Sync_Frontend/Sync_Frontend.xpr。器件必须显示xcvu11p-flgb2104-2-e，Design Sources顶层sync_frontend_top。sim_1/coarse_baseline_equivalence_tb是已通过基座测试；sim_autonomous/sync_frontend_tb是自主捕获测试。不要另建递归扫描RTL的工程。
...
最终交接提供完整核心工程、独立 VHDL wrapper 和输入/预期结果。用户可在原算法顶层手动综合和导出网表，并亲自在 LabVIEW 创建 CLIP XML/配置。既有核心综合与重开证据保留，自建 wrapper 回填不是前置门槛。正确FLGB后布线时序独立报告；NI整目标编译、真实时钟、DMA/DRAM吞吐和板测由用户完成，不外推独立OOC报告。

仿真集提醒：工程保存的默认 ActiveSimSet=sim_1，仅测试 coarse_baseline_equivalence_tb。要验证当前自主前端，在 Sources 的 Simulation Sources 中对 sim_autonomous 选择 Make Active，核对 top=sync_frontend_tb 后再由用户选择 Run Simulation。不要把默认粗核基座仿真当作自主核心完整验证。本轮保持 XPR 原字节不变，未运行仿真。
