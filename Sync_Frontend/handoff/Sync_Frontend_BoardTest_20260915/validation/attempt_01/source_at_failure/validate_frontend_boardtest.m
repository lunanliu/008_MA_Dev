...
dma=readmatrix(fullfile(baseline,'input_dma_u32.csv'));
iq=readtable(fullfile(baseline,'input_iq_view.csv'));
lanes=readmatrix(fullfile(baseline,'input_four_lanes_view.csv'));
assert(isequal(dma,reshape(lanes.',[],1)),'Lane ordering');
iu=uint32(mod(double(iq.i_i16),65536));
qu=uint32(mod(double(iq.q_i16),65536));
assert(isequal(uint32(dma),bitor(iu,bitshift(qu,16))),'CSV I/Q packing');
...
writematrix(double(reshape(records.',[],1)),exampleFile);
...
customWords=uint32(readmatrix(fullfile(custom,'input_dma_u32.csv')));
