...
iqFile=fullfile(inputDir,'input_iq.csv');
if isfile(iqFile)
    raw=readmatrix(iqFile,'Delimiter',',');
    assert(size(raw,2)==2 && all(isfinite(raw(:)) & raw(:)==fix(raw(:)) & ...
        raw(:)>=-32768 & raw(:)<=32767),'Input CSV must contain two signed I16 columns: I,Q');
    iv=int16(raw(:,1)); qv=int16(raw(:,2));
else
    % Read older output directories without modifying their frozen data.
...
stats=struct('sample_count',numel(iv),'sample_rate_hz',fs, ...
    'signal_duration_us',numel(iv)/fs*1e6,'rms_complex_code',sqrt(mean(abs(x).^2)), ...
