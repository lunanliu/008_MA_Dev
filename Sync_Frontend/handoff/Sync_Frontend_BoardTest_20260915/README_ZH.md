...
| 文件 | 用途 |
|---|---|
| data/baseline/input_iq.csv | **当前 Host 输入文件**，第一列 I、第二列 Q，逗号分隔、无表头、60324 行 |
...
当前 CSV 每行表示一个复数样点：**第一列 I，第二列 Q，中间一个逗号，无表头**。两列都是有符号 16 位整数，范围为 -32768～32767，保留原始整数幅度，不归一化。
...
Host 读出的是 N×2 数组；同一行的 I 和 Q 属于同一个时刻。由你在 Host 合并成 DMA 的 32 位数据：低 16 位 I，高 16 位 Q，负数保留二补码位型。合并后仍是一行对应一个 DMA 元素。不要将两列首尾拼接成 2N 个样点。

生成脚本现在只导出这种两列输入，不再导出预打包 U32 输入或四 lane 输入表。旧版本已生成的文件保留作为历史依据，当前选择 input_iq.csv。
...
60324 是 4 的整数倍，共 15081 次四元素读取。主输入文件有 60324 行、2 列；样点数按行数计，不是按所有单元格的总数计。
...
1. Read Delimited Spreadsheet 读取 input_iq.csv，分隔符设为逗号，得到 60324×2 数组。
2. 按行每次取 1024 行、保留两列；同一行 I/Q 由 Host 合并，得到 1024 个 32 位 DMA 元素。也可先将全表按行合并成一维数组，再按前述 For Loop 分块。
3. 将合并后的数组转换成 H2T FIFO 所需的 unsigned FXP 32/32 类型，调用 FIFO_H2T.Write，Timeout 可设 2000 ms。
...
按用户要求，当前输入改为 input_iq.csv：无表头，每行 I,Q，均为有符号 16 位十进制整数。波形内容、样点顺序、控制和结果真值不变。MATLAB 生成器与输入波形分析器已同步使用此格式；结果读回分析仍按原九字 DMA 记录解析。
