...
%GENERATE_FRONTEND_CSV Export signed I,Q columns, comma separated, no header.
...
n = numel(words);
% One complex sample per row: signed I16, signed Q16. NO HEADER.
writematrix(double([iCodes,qCodes]),fullfile(outputDir,'input_iq.csv'),'Delimiter',',');
sampleIndex = (0:n-1).';
...
info = struct('mode',mode,'sample_rate_hz',500e6,'fpga_clock_hz',125e6, ...
    ...
    'input_file','input_iq.csv', ...
    'input_format','two signed decimal I16 columns: I,Q; comma separated; no header', ...
