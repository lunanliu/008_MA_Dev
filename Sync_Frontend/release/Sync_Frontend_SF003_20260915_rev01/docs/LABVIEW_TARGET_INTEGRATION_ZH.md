...
同次EDF导出附带厂商加密EDN，不能只拿主EDF宣称完整包。独立wrapper链接/依赖检查由唯一Luna收尾，当前尚无正式完成回执。原XML使用EDF，不是DCP配置。
...
当前综合DCP可作集成候选，用户可先准备Target VI的时钟、FIFO及桥接。DCP+VHDL这个确切路径还需原生链接核查；仅EDF分支通过不等于DCP分支通过。目标VI仍必须以NI支持工具版本对整个target编译，当前未完成此步骤。
