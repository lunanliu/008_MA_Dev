...
当前交付范围：完整算法顶层 XPR/RTL + 独立 VHDL + 输入/预期结果/手工指南；XML 由用户创建。旧自动包保留为历史，当前入口 handoff/Sync_Frontend_20260915_manual/README_ZH.md。
...
技术关联由 NI 编译完成；Agent 最终交付完整工程、独立 VHDL 和手工测试资料，用户自行创建 XML；不把 wrapper 顶层 DCP 作为核心交付物。详见 docs/CLIP_NETLIST_INTEGRATION_DECISION_20260915_ZH.md。
