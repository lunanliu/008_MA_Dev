...
module fine_square18_vendor_adapter (
  input  logic clk,
  input  logic rst_n,
  ...
  t05_mult_s18x18_full36 u_mult (
    .CLK(clk),.A(req_operand),.B(req_operand),.P(product)
  );
...
module fine_square48_vendor_adapter (
  input  logic clk,
  input  logic rst_n,
  input  logic req_valid,
...
module fine_cfo_scale_vendor_adapter (
  input  logic clk,
  input  logic rst_n,
  input  logic req_valid,
  ...
  t05_mult_cfo_s19_const40_full59 u_mult (
    .CLK(clk),.A(req_a),.B(req_b),.P(rsp_product)
  );
  assign rsp_valid = valid_pipe[LATENCY-1];
