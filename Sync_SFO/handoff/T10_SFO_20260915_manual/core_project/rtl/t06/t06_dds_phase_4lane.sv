...
module t06_dds_phase_4lane (
  ...
  assign tag_agrees = tag_lane[0]==tag_lane[1] && tag_lane[0]==tag_lane[2] && tag_lane[0]==tag_lane[3];
  ...
  assign m_valid = ip_aresetn && !protocol_error_sticky && (&valid_lane) && tag_agrees && tag_lane[0][32];
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (ip_aresetn) begin
        ...
        case ({real_input_fire,real_output_fire})
        ...
        if ((real_input_fire && !real_output_fire && outstanding_real==16'hffff) ||
            (real_output_fire && outstanding_real==0)) protocol_error_sticky <= 1'b1;
      end
      if (ip_aresetn && ((valid_lane!=4'b0000 && valid_lane!=4'b1111) ||
          (ready_lane!=4'b0000 && ready_lane!=4'b1111) ||
          ((&valid_lane) && !tag_agrees))) protocol_error_sticky <= 1'b1;
    end
  end
  for (genvar lane=0; lane<4; lane++) begin : g_dds
    t06_dds_phase_s32_sincos18 u_dds (
      .aclk(clk), .aresetn(ip_aresetn),
      .s_axis_phase_tvalid(launch), .s_axis_phase_tready(ready_lane[lane]),
      .s_axis_phase_tdata(launch_dummy ? 32'b0 : s_phase[32*lane+:32]),
      .s_axis_phase_tuser(launch_dummy ? 33'b0 : {1'b1,s_tag}),
      .m_axis_data_tvalid(valid_lane[lane]), .m_axis_data_tready(retire),
      .m_axis_data_tdata(data_lane[lane]), .m_axis_data_tuser(tag_lane[lane])
    );
