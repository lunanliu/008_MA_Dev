...
module t06_observation_engine_fifo #(
    parameter integer WIDTH=96, DEPTH=32
) (
    ...
    xpm_fifo_sync #(
        ...
        .PROG_FULL_THRESH(DEPTH-8),.PROG_EMPTY_THRESH(8),
        ...
        .full(full),.prog_full(),.wr_data_count(),.overflow(overflow),
        .wr_rst_busy(wr_busy),.almost_full(),.wr_ack(),
        .rd_en(rd_en),.dout(dout),.empty(empty),.prog_empty(),.rd_data_count(),
        .underflow(underflow),.rd_rst_busy(rd_busy),.almost_empty(),.data_valid(),
