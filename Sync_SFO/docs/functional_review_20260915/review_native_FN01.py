...
for label,root in [('canonical',M),('private',P)]:
 ...
 assert sorted(actual)==sorted(expected);ips=list(csv.DictReader((R/f'{label}_ip_status.tsv').open(),delimiter='\t'));wanted_locked='1' if label=='canonical' else '0';assert len(ips)==34 and all(r['locked']==wanted_locked for r in ips);checks.append({'name':label,'native_members':len(rows),'native_members_match':True,'ip_count':34,'ip_export_locked_count':34 if label=='canonical' else 0,'lock_reason':'Read-only canonical project' if label=='canonical' else 'Private project unlocked'})
