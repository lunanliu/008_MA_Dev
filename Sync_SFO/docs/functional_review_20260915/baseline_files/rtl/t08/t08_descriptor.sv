...
module t08_descriptor #(
  parameter integer NOMINAL_SAMPLES=1336320
)(
  input logic clk,rst,
  input logic s_valid,output logic s_ready,
  input logic [31:0] s_frame_id,s_generation,s_estimate_frame_id,s_estimate_generation,
  input logic [15:0] s_estimate_status,
  input logic signed [31:0] s_ppm_q18,s_to,s_raw_first,s_available_first,s_available_last,s_nominal_first,
  ...
  always_comb begin
    input_error=0;
    if(s_frame_id!=s_estimate_frame_id || s_generation!=s_estimate_generation) input_error=8'h10;
    ...
    else if(s_input_count!=INPUT_COUNT || s_nominal_count!=NOMINAL_SAMPLES) input_error=8'h14;
    else if($signed({{32{s_raw_first[31]}},s_raw_first}) != $signed({{32{s_nominal_first[31]}},s_nominal_first})-64'sd172 ||
            s_nominal_first<0 || s_nominal_first>1336320-NOMINAL_SAMPLES) input_error=8'h15;
    else if(s_available_first>s_raw_first ||
            $signed({{32{s_available_last[31]}},s_available_last}) < $signed({{32{s_raw_first[31]}},s_raw_first})+INPUT_COUNT-1) input_error=8'h16;
  ...
  always_ff @(posedge clk)begin
    if(rst)begin
      ...
      phase_last<=0;raw64<=0;out64<=0;to64<=0;q064<=0;coordinate_term<=0;phase_bias<=0;
      ...
      IDLE:if(s_valid && s_ready)begin
        ...
        raw64<=$signed(s_raw_first);out64<=$signed(s_nominal_first);to64<=$signed(s_to);q064<=$signed({36'd0,s_q0});
      ...
      PREP:begin
        ...
        coordinate_term<=64'sd53+(to64<<<2)-(raw64<<<2);state<=COORD;
      end
      COORD:begin phase_bias<=(coordinate_term<<<28)+(q064<<<2);state<=DIVIDE;end
      DIVIDE:begin
      ...
      START_PHASE_MUL:begin
        mul_shift<=(out64<<<2)-64'sd215;mul_acc<=0;mul_bits<=m_step[28:0];bit_count<=0;state<=PHASE_MUL;
      end
      PHASE_MUL:begin
      ...
      CHECK:begin
        if(m_step<268395191 || m_step>268475721 || m_phase<0 || phase_last<m_phase ||
           first_base-64'sd107<0 || last_base+64'sd2>=4*INPUT_COUNT) m_error<=8'h17;
        state<=HOLD;
      end
      HOLD:if(m_valid && m_ready)state<=IDLE;
      default:begin m_error<=8'h18;state<=HOLD;end
    endcase
  end
  // synthesis translate_off
  initial if(NOMINAL_SAMPLES!=1336320 && NOMINAL_SAMPLES!=5120)$fatal(1,"T08 production or explicitly short module test only");
