...
module t07_nominal_pack4_rev02 #(
  parameter int unsigned SAMPLE_COUNT=1_336_392
)(
  input logic clk,rst,
  input logic s_valid,output logic s_ready,
  input logic [127:0] s_data,input logic [3:0] s_mask,
  output logic m_valid,input logic m_ready,output logic [127:0] m_data,
  output logic [31:0] m_beat,output logic m_last,
  output logic [31:0] formed_beats,physical_beats,
  ...
  always_ff @(posedge clk) begin
    if(rst) begin
      carry<='0;carry_valid<=1'b0;pending_valid<=1'b0;m_data<='0;
      m_beat<='0;m_last<=1'b0;formed_beats<='0;physical_beats<='0;error_sticky<=1'b0;
    end else begin
      ...
      if(s_valid && s_ready) begin
        ...
        case(s_mask)
          ...
          4'b1000:begin
            ...
            carry<=s_data[127:96];carry_valid<=1'b1;
          end
          4'b1111,4'b0111:begin
            if(!carry_valid || formed_beats>=OUTPUT_BEATS)error_sticky<=1'b1;
            m_data<={s_data[95:0],carry};pending_valid<=1'b1;
            m_beat<=formed_beats;m_last<=(formed_beats==OUTPUT_BEATS-1);
            formed_beats<=formed_beats+1'b1;
            carry<=s_data[127:96];carry_valid<=(s_mask==4'b1111);
          end
