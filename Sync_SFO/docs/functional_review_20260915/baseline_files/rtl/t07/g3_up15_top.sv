...
// The one-entry elastic register clips the guarded result to signed16.
module g3_up15_top #(
 parameter integer IT=8, OT=16,
 parameter integer GW=17, GS=8*((GW+7)/8)
)(
 input wire aclk,aresetn,s_axis_data_tvalid,
 ...
 genvar lane;
 generate for(lane=0;lane<IT;lane=lane+1)begin:input_unpack
  assign i_in[lane*16+:16]=s_axis_data_tdata[(lane*2)*16+:16];
  assign q_in[lane*16+:16]=s_axis_data_tdata[(lane*2+1)*16+:16];
 end for(lane=0;lane<OT;lane=lane+1)begin:output_pack
  assign packed_guarded[(lane*2)*GS+:GS]=i_out[lane*GS+:GS];
  assign packed_guarded[(lane*2+1)*GS+:GS]=q_out[lane*GS+:GS];
 end endgenerate
 t07_g3_up15 i_core(.aclk(aclk),.aresetn(aresetn),.s_axis_data_tvalid(s_axis_data_tvalid),.s_axis_data_tready(ri),.s_axis_data_tdata(i_in),.m_axis_data_tready(take_output),.m_axis_data_tvalid(vi),.m_axis_data_tdata(i_out));
 t07_g3_up15 q_core(.aclk(aclk),.aresetn(aresetn),.s_axis_data_tvalid(s_axis_data_tvalid),.s_axis_data_tready(rq),.s_axis_data_tdata(q_in),.m_axis_data_tready(take_output),.m_axis_data_tvalid(vq),.m_axis_data_tdata(q_out));
 ...
 always @(posedge aclk)begin
  if(!aresetn)begin
   m_axis_data_tvalid<=0;m_axis_data_tdata<=0;debug_guarded_data<=0;saturation<=0;
