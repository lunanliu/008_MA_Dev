`timescale 1ns/1ps
`include "t06_pair_max_tracker_2lane_ip_config.svh"
// Exact rational maximum. No quotient approximation or observation storage.
module t06_pair_max_tracker_2lane (
    input logic clk,rst,
    input logic pair_start_valid, output logic pair_start_ready,
    input logic [31:0] pair_start_tag,
    input logic s_valid, output logic s_ready,
    input logic [63:0] s_nw0,s_nw1,
    input logic [32:0] s_dw0,s_dw1,
    ...
    localparam integer CROSS_LAT=`T06_PAIR_MAX_CROSS_LATENCY;
    typedef enum logic [2:0] {IDLE,FEED,DRAIN,MERGE_ISSUE,MERGE_WAIT,EMIT,OUT} state_t;
    typedef struct packed {logic [63:0] n; logic [32:0] d; logic [10:0] index;} candidate_t;
    ...
    function automatic candidate_t choose(input meta_t entry,input logic [96:0] left_product,right_product);
    ...
    genvar g;
    generate for (g=0;g<2;g=g+1) begin:cross_lane
        t06_pair_max_tracker_2lane_cross left_product (
            .CLK(clk),.A(proposed[g].n),.B(incumbent[g].d),.P(lhs[g]));
        t06_pair_max_tracker_2lane_cross right_product (
            .CLK(clk),.A(incumbent[g].n),.B(proposed[g].d),.P(rhs[g]));
    end endgenerate

    always_ff @(posedge clk) begin
        if (rst) begin
            state<=IDLE;beat_count<=0;merge_index<=0;pair_error<=0;
            active_tag<=0;slot_busy<=0;best<='0;
            m_nm<=0;m_dm<=0;m_index<=0;m_tag<=0;m_pair_valid<=0;m_error_code<=0;
            ...
            for (lane=0;lane<2;lane=lane+1) begin
                ...
                if (pipe_valid[lane][CROSS_LAT-1]) begin
                    if (meta[lane][CROSS_LAT-1].merging) begin
                        ...
                        else begin merge_index<=merge_index+1'b1;state<=MERGE_ISSUE;end
                    end else begin
            ...
            case (state)
                IDLE: if (pair_start_valid) begin
                    state<=FEED;active_tag<=pair_start_tag;beat_count<=0;
                    pair_error<=0;best<='0;merge_index<=0;slot_busy<=0;
                    for (slot=0;slot<16;slot=slot+1) partial[slot]<='0;
                end
                FEED: if (accept) begin
                    if (s_index0!={beat_count,1'b0} || s_index1!={beat_count,1'b1} ||
                        (s_nw0!=0 && s_dw0==0) || (s_nw1!=0 && s_dw1==0)) pair_error<=1'b1;
                    if (beat_count==819) state<=DRAIN;
                    else beat_count<=beat_count+1'b1;
                end
                DRAIN: if (pipe_valid[0]=='0 && pipe_valid[1]=='0 && slot_busy==0) begin
                    state<=MERGE_ISSUE;merge_index<=0;best<='0;
                end
                MERGE_ISSUE: state<=MERGE_WAIT;
                MERGE_WAIT: begin end
                EMIT: begin
                    m_tag<=active_tag;
    ...
    always @(posedge clk) if (!rst) begin
        for (integer a=0;a<2;a=a+1) begin
            if (issue[a] && state==FEED && slot_busy[issue_slot[a]])
                $fatal(1,"PAIR_MAX slot reused before previous commit lane=%0d slot=%0d",a,issue_slot[a]);
            if (pipe_valid[a][CROSS_LAT-1] && meta[a][CROSS_LAT-1].merging && (a!=0 || state!=MERGE_WAIT))
                $fatal(1,"PAIR_MAX merge metadata/state mismatch");
            if (pipe_valid[a][CROSS_LAT-1] && !meta[a][CROSS_LAT-1].merging && !slot_busy[meta[a][CROSS_LAT-1].slot])
        ...
        if ((state==IDLE || state==OUT) && (pipe_valid[0]!='0 || pipe_valid[1]!='0 || slot_busy!=0))
