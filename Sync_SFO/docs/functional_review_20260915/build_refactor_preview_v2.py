...
M=Path('D:/008_MA_Dev/Sync_SFO');A=M/'docs/functional_review_20260915';B=A/'baseline_files';V=M/'work/functional_review_tools_20260915/package/verible-v0.0-4214-gce503962-win64';P=M/'work/functional_refactor_preview_v2_20260915';P.mkdir(exist_ok=False)
...
(A/'preview_equivalence_v2.json').write_text(json.dumps(summary,indent=2),encoding='utf-8');print(json.dumps({'status':summary['status'],'passed':len(results),'count':len(sv),'expanded_ports':summary['expanded_inherited_ports'],'failures':[{'file':r['file'],'error':r['error'][:350]} for r in failures]}));assert not failures
