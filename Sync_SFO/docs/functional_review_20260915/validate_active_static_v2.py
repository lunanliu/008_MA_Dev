  ...
  for gate in nodes(inst):
   ...
   for port in nodes(gate):
    if port.get('tag')=='kActualNamedPort':
     name_token=leaves(port)[1];p=raw[name_token['start']:name_token['end']].decode();names.append(p);need(p in mods[typ],f'Unknown named port {rel}:{typ}.{p}');connections+=1
...
def vhports(kind,name):
 start=re.search(r'\b'+kind+r'\s+'+name+r'\s+is\s+port\s*\(',vhd,re.S|re.I).end();depth=1;end=start
 while depth:
  depth+=int(vhd[end]=='(')-int(vhd[end]==')');end+=1
 s=vhd[start:end-1];found=[]
