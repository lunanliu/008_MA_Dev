...
import json,re,subprocess,collections,concurrent.futures,hashlib,sys
M=Path('D:/008_MA_Dev/Sync_SFO');A=M/'docs/functional_review_20260915';B=A/'baseline_files';V=M/'work/functional_review_tools_20260915/package/verible-v0.0-4214-gce503962-win64';P=M/'work'/sys.argv[1];P.mkdir(exist_ok=False)
...
def parse(raw,key,tree=True):
 ...
 args=[str(V/'verible-verilog-syntax.exe'),'--export_json','--printtokens','--printrawtokens']
...
def modules(d):
 ...
 return out
cached={rel:parse((B/rel).read_bytes(),'baseline_cache/'+rel) for rel in plan['paths'] if Path(rel).suffix in ('.sv','.svh')}
all_words=set()
...
def rewrite(rel,raw,d):
 ...
 for mod,start,end,head in modules(d):
  for n in nodes(d['tree']):
   if n.get('tag')=='kGateInstance':
    ident=n['children'][0];name=ident.get('text')
    if start<=ident.get('start',-1)<end and name in scoped.get(mod,{}):
     replacements[ident['start']]=(ident['end'],scoped[mod][name].encode())
...
def normalized_raw_tokens(d,raw,restore=False):
 result=[]
 for t in d['rawtokens']:
  word=texttok(t,raw)
  if not word.strip():continue
  if t['tag']=='PP_define_body':word=word.rstrip()
  if restore:word=restore_word(word)
  result.append((t['tag'],word))
 return result
...
def process(rel):
 baseline_bytes=(B/rel).read_bytes();raw=baseline_bytes.replace(b'\r\n',b'\n');d=parse(raw,'baseline/'+rel);renamed,expanded,ports,exp_records=rewrite(rel,raw,d)
 new=plan['paths'][rel];draft=P/'unformatted'/new;draft.parent.mkdir(parents=True,exist_ok=True);draft.write_bytes(renamed)
 fmt=subprocess.run([str(V/'verible-verilog-format.exe'),'--column_limit=100','--indentation_spaces=2','--port_declarations_alignment=align','--named_port_alignment=align','--try_wrap_long_lines=true','--failsafe_success=false','--max_search_states=4000000',str(draft)],capture_output=True)
 if fmt.returncode:raise RuntimeError('Formatter '+rel+': '+fmt.stderr.decode(errors='replace'))
 formatted=fmt.stdout.replace(b'\r\n',b'\n').rstrip(b'\n')+b'\n';target=P/'ready'/new;target.parent.mkdir(parents=True,exist_ok=True);target.write_bytes(formatted)
 ...
 assert normalized_raw_tokens(ed,expanded)==normalized_raw_tokens(nd,formatted,True),'Inverse raw-token mismatch '+rel
 ...
 return {'old':rel,'new':new,'before_sha256':hashlib.sha256(baseline_bytes).hexdigest(),'after_sha256':hashlib.sha256(formatted).hexdigest(),'inverse_tokens_equal':True,'inverse_raw_tokens_equal':True,'inverse_tree_equal':True,'ports_equal':True,'expanded_ports':exp_records,'lines_before':len(raw.splitlines()),'lines_after':len(formatted.splitlines()),'max_line_after':max(map(len,formatted.splitlines()),default=0),'formatter_stderr':fmt.stderr.decode(errors='replace')}
...
(P/'used_rename_plan.json').write_text(json.dumps(plan,ensure_ascii=False,indent=2),encoding='utf-8')
...
(P/'reproduced_equivalence.json').write_text(json.dumps(summary,indent=2),encoding='utf-8');print(json.dumps({'status':summary['status'],'passed':len(results),'count':len(sv),'expanded_ports':summary['expanded_inherited_ports'],'failures':[{'file':r['file'],'error':r['error'][:350]} for r in failures]}));assert not failures
