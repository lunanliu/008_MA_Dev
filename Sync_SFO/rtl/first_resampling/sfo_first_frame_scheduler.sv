...
module sfo_first_frame_scheduler #(
  ...
  wire normal_active = state == CLEAR_CORE || state == START_CORE || state == INIT_PHASE ||
      state == RUN_FRAME || state == QUIET_TAIL;
  wire faultable = normal_active || state == COMPLETE_FRAME;
  wire local_reset = rst || state == CLEAR_CORE || state == ABORT_CORE_RESET ||
      state == COMPLETE_ERROR || state == HALTED;
  ...
  assign m_data = pack_data;
  assign m_beat = pack_beat;
  assign m_last = pack_last;
  assign m_frame_id = frame_id;
  assign m_generation = generation;
  assign completed_frame_id = frame_id;
  assign completed_generation = generation;
  assign completion_valid = !rst &&
      ((state == COMPLETE_FRAME && !abort_request) || state == COMPLETE_ERROR);
    ...
    if (faultable) begin
      if (abort_request) fault_code = 8'd1;
        ...
        if (pack_error) fault_code = 8'd2;
          ...
          for (int i = 0; i < 6; i++)
          if (observed_accept[i] &&
              accepted_stage_beats[i] >= (i < 3 ? INPUT_BEATS : REQUEST_BEATS))
            fault_code = 8'd3;
          if (fault_code == 0 && s_valid && s_ready &&
              (s_frame_id != frame_id || s_generation != generation ||
               s_beat != accepted_stage_beats[0]))
            fault_code = 8'd4;
          if (fault_code == 0 && s_valid && s_ready &&
              (s_lane_valid != 4'hf || s_last != (accepted_stage_beats[0] == INPUT_BEATS - 1) ||
               $signed(
                  {{32{s_raw_index[31]}}, s_raw_index}
              ) != raw_first + ($signed(
                  {32'd0, accepted_stage_beats[0]}
              ) <<< 2)))
            fault_code = 8'h20;
          if (fault_code == 0 && m_valid && m_ready &&
              (m_beat != accepted_packed_beats ||
               m_last != (accepted_packed_beats == PACKED_BEATS - 1)))
            fault_code = 8'd5;
        end
      end else if (state == QUIET_TAIL && (observed_valid != 0 || !core_done || !arithmetic_empty ||
                                           pack_valid || pack_error))
        fault_code = 8'd6;
    end
  end
  sfo_guarded_resampling_core #(
      ...
      .start                    (core_start),
      .cfg_step                 (step),
      .cfg_request_beats        (REQUEST_BEATS),
      .cfg_phase0               (phase0),
      .s_valid                  (core_input_valid),
      .s_ready                  (core_input_ready),
      .s_data                   (s_data),
      .m_valid                  (core_valid),
      .m_ready                  (core_ready),
      .m_data                   (core_data),
      .m_beat                   (core_beat),
      .m_nominal_mask           (core_mask),
      .done                     (core_done),
      .observed_valid           (observed_valid),
      .observed_accept          (observed_accept),
      .observed_arithmetic_empty(arithmetic_empty)
  );
  sfo_nominal_pack4 #(
      ...
      .clk           (clk),
      .rst           (local_reset),
      .s_valid       (run_enable && core_valid),
      .s_ready       (pack_ready),
      .s_data        (core_data),
      .s_mask        (core_mask),
      .m_valid       (pack_valid),
      .m_ready       (run_enable && m_ready),
      .m_data        (pack_data),
      .m_beat        (pack_beat),
      .m_last        (pack_last),
      .formed_beats  (formed_beats),
      .physical_beats(physical_beats),
      .error_sticky  (pack_error)
  );
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      raw_first <= '0;
      processing_cycles <= '0;
      accepted_stage_beats <= '0;
      accepted_packed_beats <= '0;
      error_sticky <= 1'b0;
      first_error_code <= '0;
      first_error_cycle <= '0;
    end else begin
      ...
      if (state == RUN_FRAME) begin
        for (int i = 0; i < 6; i++)
        if (observed_accept[i]) accepted_stage_beats[i] <= accepted_stage_beats[i] + 1'b1;
        if (m_valid && m_ready) accepted_packed_beats <= accepted_packed_beats + 1'b1;
      end
      if (faultable && fault_code != 0) begin
        error_sticky <= 1'b1;
        first_error_code <= fault_code;
        first_error_cycle <= processing_cycles;
        phase_counter <= '0;
        state <= ABORT_CORE_RESET;
      end else
        case (state)
          IDLE:
          if (cfg_valid && cfg_ready) begin
            frame_id <= cfg_frame_id;
            generation <= cfg_generation;
            step <= cfg_step;
            phase0 <= cfg_phase0;
            raw_first <= $signed(cfg_raw_first);
            processing_cycles <= '0;
            accepted_stage_beats <= '0;
            accepted_packed_beats <= '0;
            phase_counter <= '0;
          ...
          CLEAR_CORE:
          if (phase_counter == 31) begin
            phase_counter <= '0;
            state <= START_CORE;
          end else phase_counter <= phase_counter + 1'b1;
          START_CORE: begin
            phase_counter <= '0;
            state <= INIT_PHASE;
          end
          INIT_PHASE:
          if (phase_counter == 15) begin
            phase_counter <= '0;
            state <= RUN_FRAME;
          end else phase_counter <= phase_counter + 1'b1;
          RUN_FRAME:
          if (transport_complete && core_done && arithmetic_empty && !pack_valid &&
              accepted_packed_beats == PACKED_BEATS && formed_beats == PACKED_BEATS &&
              physical_beats == REQUEST_BEATS) begin
            phase_counter <= '0;
            state <= QUIET_TAIL;
          end
          QUIET_TAIL:
          if (phase_counter == 100) state <= COMPLETE_FRAME;
          else phase_counter <= phase_counter + 1'b1;
          COMPLETE_FRAME: if (completion_valid && completion_ready) state <= IDLE;
          ABORT_CORE_RESET:
          if (phase_counter == 31) state <= COMPLETE_ERROR;
          else phase_counter <= phase_counter + 1'b1;
