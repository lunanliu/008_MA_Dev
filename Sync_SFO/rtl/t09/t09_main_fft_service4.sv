// T09 main FFT: same T03 forward arithmetic, full-window output reservation and synchronized completion/error status.
`timescale 1ns/1ps

module t09_main_fft_service4 #(
  ...
  xpm_cdc_sync_rst #(.DEST_SYNC_FF(4),.INIT(1),.INIT_SYNC_FF(1),.SIM_ASSERT_CHK(1))
    reset_sync500(.src_rst(reset_source125),.dest_clk(clk_500),.dest_rst(reset500));
  ...
  localparam logic [15:0] FORWARD_SCALE_CONFIG = 16'h0D57;
  ...
  logic [7:0] xfft_output_user,xfft_status_data;
  logic xfft_status_valid;
  logic [12:0] out_fifo_wr_count;
  logic [10:0] out_fifo_rd_count;
  logic status_toggle,status_last;
  (* ASYNC_REG="TRUE" *) logic [1:0] status_sync;
  (* ASYNC_REG="TRUE" *) logic [3:0] errors_meta,errors_sync;
  logic egress_overflow_500,ingress_underflow_500,xfft_protocol_error_500,xfft_overflow_500;
  ...
  always_ff @(posedge clk_500) begin
    if (reset500) begin
      ...
      if (!configured && xfft_config_ready)
        configured <= 1'b1;
      ...
      else if (in_fifo_rd_en && in_fifo_dout[32])
        fft_transaction_active <= 1'b0;
    end
  end

  assign in_fifo_rd_en = !reset500 && !in_fifo_rd_busy && fft_transaction_active && !in_fifo_empty &&
                         in_fifo_data_valid && xfft_input_ready;
  ...
  t03_xfft_2048_main u_xfft (
    ...
    .s_axis_data_tvalid(!reset500 && !in_fifo_rd_busy && fft_transaction_active && !in_fifo_empty && in_fifo_data_valid),
    .s_axis_data_tready(xfft_input_ready),
    .s_axis_data_tlast(in_fifo_dout[32]),
    .m_axis_data_tdata(xfft_output_data),
    .m_axis_data_tuser(xfft_output_user),
    .m_axis_status_tdata(xfft_status_data),
    .m_axis_status_tvalid(xfft_status_valid),
  ...
  xpm_fifo_async #(
    ...
    .full(out_fifo_full),
    .prog_full(),
    .wr_data_count(out_fifo_wr_count),
    ...
    .prog_empty(),
    .rd_data_count(out_fifo_rd_count),
  ...
  assign m_transaction_end = |{out_fifo_dout[131], out_fifo_dout[98],
                               out_fifo_dout[65], out_fifo_dout[32]};

  always_ff @(posedge clk_500) begin
    if (reset500) begin
      egress_overflow_500 <= 1'b0;
      ingress_underflow_500 <= 1'b0;
      xfft_protocol_error_500 <= 1'b0;
      xfft_overflow_500 <= 1'b0;
    end else begin
      if (out_fifo_overflow || (xfft_output_valid && (out_fifo_full || out_fifo_wr_busy)))
        egress_overflow_500 <= 1'b1;
      if (in_fifo_underflow)
        ingress_underflow_500 <= 1'b1;
      if (event_tlast_unexpected || event_tlast_missing || event_data_in_channel_halt)
        xfft_protocol_error_500 <= 1'b1;
      ...
      if (event_fft_overflow || (xfft_output_valid && xfft_output_user[0]) ||
          (xfft_status_valid && (xfft_status_data != 0)))
        xfft_overflow_500 <= 1'b1;
    end
  end
  ...
  always_ff @(posedge clk_125) begin
    if (!rst_n) begin
      ...
      if (in_fifo_overflow)
        ingress_overflow <= 1'b1;
      if (out_fifo_underflow)
        egress_underflow <= 1'b1;
    end
  end

  // These slow-clock outputs carry sticky status, never unsynchronized pulses.
  // Exactly one transaction is admitted by the caller until all output and status
  // are consumed. 4096 narrow entries reserve more than a full 2048-point result.
  assign {egress_overflow,ingress_underflow,xfft_protocol_error,xfft_overflow}=errors_sync;
  assign status_valid=status_sync[1] ^ status_last;
  always_ff @(posedge clk_500) begin
    if(reset500)status_toggle<=0;
    else if(xfft_status_valid)status_toggle<=~status_toggle;
  end
  always_ff @(posedge clk_125) begin
    if(!rst_n)begin errors_meta<=0;errors_sync<=0;status_sync<=0;status_last<=0;end
    else begin
      errors_meta<={egress_overflow_500,ingress_underflow_500,xfft_protocol_error_500,xfft_overflow_500};
      errors_sync<=errors_meta;
      status_sync<={status_sync[0],status_toggle};status_last<=status_sync[1];
    end
  end
endmodule
