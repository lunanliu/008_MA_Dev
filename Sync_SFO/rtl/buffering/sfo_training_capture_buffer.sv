...
module sfo_training_capture_buffer #(
  ...
  initial begin
    ...
    if ((1 << ADDR_WIDTH) < DEPTH_BEATS) $error("T05 local capture ADDR_WIDTH is too small");
  end
  ...
  always_comb begin
    ...
    write_enable = tap_fire && tap_in_range && tap_aligned && (!capture_valid || capture_active);
    write_addr = tap_relative_addr;
    read_req_ready = capture_valid && (read_req_addr < DEPTH_BEATS[ADDR_WIDTH-1:0]);
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      if (read_req_valid && !read_req_ready) read_protocol_error_sticky <= 1'b1;
      ...
      if (tap_fire && tap_in_range) begin
        if (capture_valid && !capture_active) begin
          ...
          active_context_valid <= tap_aligned && (tap_relative_addr == 0) &&
              (tap_lane_valid == 4'hf);
          if (!tap_aligned || tap_relative_addr != 0 || tap_lane_valid != 4'hf)
            capture_protocol_error_sticky <= 1'b1;
          if (tap_relative_addr == DEPTH_BEATS - 1) begin
            capture_active <= 1'b0;
            capture_valid <= 1'b1;
            capture_context_valid <= tap_aligned && (tap_relative_addr == 0) &&
                (tap_lane_valid == 4'hf);
            capture_frame_id <= tap_frame_id;
          end
        end else begin
          if (!tap_aligned || tap_frame_id != active_frame_id ||
              tap_relative_addr != expected_addr || tap_lane_valid != 4'hf) begin
            active_context_valid <= 1'b0;
            capture_protocol_error_sticky <= 1'b1;
          end
          expected_addr <= tap_relative_addr + 1'b1;
          if (tap_relative_addr == DEPTH_BEATS - 1) begin
            ...
            capture_context_valid <= active_context_valid && tap_aligned &&
                (tap_frame_id == active_frame_id) && (tap_relative_addr == expected_addr) &&
                (tap_lane_valid == 4'hf);
