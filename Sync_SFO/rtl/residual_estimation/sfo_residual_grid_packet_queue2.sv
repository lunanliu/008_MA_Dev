...
module sfo_residual_grid_packet_queue2 (
    input  logic         clk,
    input  logic         rst,
    input  logic         alloc_valid,
    output logic         alloc_ready,
    input  logic [ 70:0] alloc_identity,
    input  logic         s_valid,
    output logic         s_ready,
    input  logic [208:0] s_word,
    input  logic         commit_valid,
    output logic         commit_ready,
    output logic         packet_valid,
    output logic [ 70:0] packet_identity,
    output logic         m_valid,
    input  logic         m_ready,
    output logic [208:0] m_word,
    output logic [  1:0] committed_count,
    output logic         fill_active,
    output logic [  1:0] reserved_count,
    output logic         error
);
  ...
  wire input_bad = s_word[208:138] != identity[wr_slot] || s_word[137:129] != fill_count[8:0] ||
      s_word[128] != (fill_count == 511);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      if (allocate) begin
        fill_active <= 1;
        fill_count <= 0;
        identity[wr_slot] <= alloc_identity;
      end
      if (write_fire) begin
