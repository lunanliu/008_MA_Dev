...
module sfo_residual_pilot_rotate2 (
    input  logic        clk,
    input  logic        rst,
    input  logic        s_valid,
    output logic        s_ready,
    input  logic [63:0] s_data,
    input  logic [ 5:0] s_phase,
    input  logic [ 1:0] s_mask,
    input  logic [31:0] s_tag,
    output logic        m_valid,
    input  logic        m_ready,
    output logic [71:0] m_data,
    output logic [ 1:0] m_mask,
  ...
  function automatic signed [17:0] pre_rotate(input logic signed [15:0] a,
                                              input logic signed [15:0] b, input logic [2:0] phase,
                                              input logic imaginary);
  ...
  function automatic signed [34:0] scale_pre(input logic signed [17:0] value,
                                             input logic odd_phase);
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      for (int lane = 0; lane < 2; lane++) begin
        ...
        pre_i[lane] <= s_mask[lane] ? pre_rotate(
            $signed(s_data[lane*32+:16]), $signed(s_data[lane*32+16+:16]), s_phase[lane*3+:3], 1'b0
        ) : 18'sd0;
        pre_q[lane] <= s_mask[lane] ? pre_rotate(
            $signed(s_data[lane*32+:16]), $signed(s_data[lane*32+16+:16]), s_phase[lane*3+:3], 1'b1
        ) : 18'sd0;
