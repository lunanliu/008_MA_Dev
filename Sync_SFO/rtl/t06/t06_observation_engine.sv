`timescale 1ns/1ps
`include "t06_observation_engine_ip_config.svh"
...
module t06_observation_engine (
    input logic clk,rst,s_valid, output logic s_ready,
    input logic [31:0] s_first_iq,s_second_iq,s_tag,
    output logic m_valid, input logic m_ready,
    output logic signed [47:0] m_phase,m_native_phase,
    output logic [31:0] m_power_first,m_power_second,
    output logic [63:0] m_weight_numerator,
    output logic [32:0] m_weight_denominator,
    output logic [31:0] m_tag,
    output logic m_zero_power,m_axis_mapped,m_error,
    output logic [7:0] pending_count,
    output logic protocol_error_sticky
);
    localparam integer SQUARE_LAT=`T06_OBS_SQUARE_LATENCY;
    localparam integer WEIGHT_LAT=`T06_OBS_WEIGHT_LATENCY;
    ...
    logic output_full,output_empty,output_wr_busy,output_rd_busy,output_overflow,output_underflow;
    ...
    assign can_admit=!rst && !protocol_error_sticky && !input_full &&
        !input_wr_busy && !input_rd_busy && !output_wr_busy && !output_rd_busy &&
        (pending_count<128);
    assign s_ready=can_admit && pair_ready;
    assign accept=s_valid && s_ready;
    t06_pair_phasor_bfp pair_bfp (
        .clk(clk),.rst(rst),.s_valid(s_valid && can_admit),.s_ready(pair_ready),
        .s_first_iq(s_first_iq),.s_second_iq(s_second_iq),.s_tag(s_tag),
        .m_valid(pair_valid),.m_ready(pair_output_ready),
        .m_phasor_re(pair_re),.m_phasor_im(pair_im),
        .m_bfp_re(pair_bfp_re),.m_bfp_im(pair_bfp_im),.m_shift(pair_shift),
        .m_zero(pair_zero),.m_error(pair_error),.m_tag(pair_tag),
        .pending_count(pair_pending),.protocol_error_sticky(pair_protocol));
    assign pair_output_ready=!rst && !input_empty && !input_rd_busy && !output_wr_busy;
    assign launch=pair_valid && pair_output_ready;
    t06_observation_engine_fifo #(.WIDTH($bits(input_t)),.DEPTH(32)) input_fifo (
        .clk(clk),.rst(rst),.wr_en(accept),.rd_en(launch),
        .din({s_tag,s_second_iq,s_first_iq}),.dout(input_head),
        .full(input_full),.empty(input_empty),.wr_busy(input_wr_busy),.rd_busy(input_rd_busy),
        .overflow(input_overflow),.underflow(input_underflow));
    ...
    t06_cordic_translate48_parallel translate (
        .aclk(clk),.aresetn(!rst),.s_axis_cartesian_tvalid(launch),
        .s_axis_cartesian_tdata(cordic_cartesian),
        .m_axis_dout_tvalid(cordic_valid),.m_axis_dout_tdata(cordic_result));
    t06_observation_engine_square square_i1 (.CLK(clk),.A(input_head.first_iq[15:0]),.B(input_head.first_iq[15:0]),.P(squares[0]));
    t06_observation_engine_square square_q1 (.CLK(clk),.A(input_head.first_iq[31:16]),.B(input_head.first_iq[31:16]),.P(squares[1]));
    t06_observation_engine_square square_i2 (.CLK(clk),.A(input_head.second_iq[15:0]),.B(input_head.second_iq[15:0]),.P(squares[2]));
    t06_observation_engine_square square_q2 (.CLK(clk),.A(input_head.second_iq[31:16]),.B(input_head.second_iq[31:16]),.P(squares[3]));
    ...
    t06_observation_engine_weight_product product (
        .CLK(clk),.A(power1_reg),.B(power2_reg),.P(weight_product));

    always_ff @(posedge clk) begin
        if(rst) begin
            pending_count<=0;protocol_error_sticky<=0;
            observation_valid<='0;zero_pipe<='0;axis_pipe<='0;error_pipe<='0;
            square_valid<='0;power_valid<=0;product_valid<='0;aligned_valid<='0;
            power1_reg<=0;power2_reg<=0;denominator_reg<=0;
            ...
            power1_reg<=power1_sum[31:0];power2_reg<=power2_sum[31:0];
            ...
            for(stage=1;stage<WEIGHT_LAT;stage=stage+1) power_meta[stage]<=power_meta[stage-1];
            ...
            for(stage=1;stage<ALIGN_LAT;stage=stage+1) aligned_weight[stage]<=aligned_weight[stage-1];
            if(pair_protocol || input_overflow || input_underflow || output_overflow || output_underflow ||
                (launch && (pair_tag!=input_head.tag || pair_zero!=((input_head.first_iq==0)||(input_head.second_iq==0)))) ||
                (cordic_valid!=observation_valid[CORDIC_LAT-1]) ||
                (aligned_valid[ALIGN_LAT-1]!=observation_valid[CORDIC_LAT-1])) protocol_error_sticky<=1;
        end
    end
    always_comb begin
        output_word='0;
        ...
        if(output_word.native_phase>=PHASE_ONE) output_word.phase=output_word.native_phase-PHASE_TWO;
        else if(output_word.native_phase < -PHASE_ONE) output_word.phase=output_word.native_phase+PHASE_TWO;
        ...
        output_word.error=error_pipe[CORDIC_LAT-1] ||
            output_word.phase < -PHASE_ONE || output_word.phase>=PHASE_ONE;
    end
    assign output_write=!rst && !output_wr_busy && observation_valid[CORDIC_LAT-1] && cordic_valid;
    t06_observation_engine_fifo #(.WIDTH($bits(output_t)),.DEPTH(128)) result_fifo (
        .clk(clk),.rst(rst),.wr_en(output_write),.rd_en(consume),.din(output_word),.dout(output_head),
        .full(output_full),.empty(output_empty),.wr_busy(output_wr_busy),.rd_busy(output_rd_busy),
        .overflow(output_overflow),.underflow(output_underflow));
    assign m_phase=output_head.phase;assign m_native_phase=output_head.native_phase;
    assign m_power_first=output_head.p1;assign m_power_second=output_head.p2;
    assign m_weight_numerator=output_head.nw;assign m_weight_denominator=output_head.dw;
    assign m_tag=output_head.tag;assign m_zero_power=output_head.zero_power;
    assign m_axis_mapped=output_head.axis_mapped;assign m_error=output_head.error;
`ifndef SYNTHESIS
    initial if(SQUARE_LAT!=3 || WEIGHT_LAT!=4 || ALIGN_LAT!=49) $fatal(1,"Observation IP schedule mismatch");
    always @(posedge clk) if(!rst) begin
        if(pending_count>128 || (consume && pending_count==0) || protocol_error_sticky)
            $fatal(1,"Observation credit/protocol invariant");
        if(launch && (pair_tag!==input_head.tag || pair_error || pair_zero!==((input_head.first_iq==0)||(input_head.second_iq==0))))
            $fatal(1,"Observation pair/power association mismatch");
        if(square_valid[SQUARE_LAT-1] && (squares[0][31] || squares[1][31] || squares[2][31] || squares[3][31] || power1_sum[32] || power2_sum[32]))
            $fatal(1,"Observation square/power exact range violation");
        if(cordic_valid!==observation_valid[CORDIC_LAT-1] || aligned_valid[ALIGN_LAT-1]!==observation_valid[CORDIC_LAT-1])
            $fatal(1,"Observation CORDIC/power latency mismatch");
        if(output_write && (output_full || output_word.error || (output_word.zero_power && (output_word.nw!=0 || output_word.phase!=0))))
