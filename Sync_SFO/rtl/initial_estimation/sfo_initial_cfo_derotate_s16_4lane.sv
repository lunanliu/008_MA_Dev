...
module sfo_initial_cfo_derotate_s16_4lane (
    input  logic         clk,
    input  logic         rst,
    input  logic         s_valid,
    output logic         s_ready,
    input  logic [127:0] s_phase,
    input  logic [ 63:0] s_i,
    input  logic [ 63:0] s_q,
  ...
  logic gain_valid;
  assign raw_available = !rst && !raw_wr_busy && !raw_rd_busy && !raw_full &&
      raw_pending_count < RAW_DEPTH;
  ...
  assign
      join_valid = dds_valid && !raw_empty && !raw_rd_busy && tag_match && !protocol_error_sticky;
  assign
      dds_ready = join_ready && !raw_empty && !raw_rd_busy && tag_match && !protocol_error_sticky;
  assign raw_pop = join_valid && join_ready;
  sfo_initial_dds_phase_4lane u_dds (
      .clk                  (clk),
      .rst_n                (!rst),
      .s_valid              (dds_s_valid),
      .s_ready              (dds_s_ready),
      ...
      .m_sine               (dds_sin),
      .m_tag                (dds_tag),
      .protocol_error_sticky(dds_error)
  );
  sfo_initial_derotate_wide_4lane u_rotate (
      .clk                  (clk),
      .rst                  (rst),
      .s_valid              (join_valid),
      .s_ready              (join_ready),
      ...
      .m_tag                (wide_tag),
      .m_error              (wide_error),
      .pending_count        (wide_pending),
      .protocol_error_sticky(wide_protocol_error)
  );
  sfo_initial_shared_gain_s16_4lane u_gain (
      .clk           (clk),
      .rst           (rst),
      .s_valid       (wide_valid && !protocol_error_sticky),
      .s_ready       (wide_ready),
      .s_i           (wide_i),
      .s_q           (wide_q),
      .s_tag         (wide_tag),
      .m_valid       (gain_valid),
      .m_ready       (m_ready && !protocol_error_sticky),
      .m_i           (m_i),
      .m_q           (m_q),
      .m_tag         (m_tag),
      .m_scaled      (m_scaled),
      .m_limiting_i  (m_limiting_i),
      .m_s18_overflow(m_s18_overflow),
      .m_range_error (m_range_error)
  );
  ...
  xpm_fifo_sync #(
      ...
      .prog_empty   (),
      .rd_data_count(),
      .underflow    (raw_underflow),
      .rd_rst_busy  (raw_rd_busy),
      .almost_empty (),
      .data_valid   (),
      .injectsbiterr(1'b0),
      .injectdbiterr(1'b0),
