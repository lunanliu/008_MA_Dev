...
module sfo_cfo_phase_increment #(
  ...
  function automatic [32:0] rne_shift35_to_signed32(input logic signed [58:0] value);
    logic sign_value;
    ...
    begin
      ...
      magnitude = sign_value ? $unsigned(-extended_value) : $unsigned(extended_value);
      quotient = magnitude >> 35;
      remainder = magnitude[34:0];
      increment = (remainder > 35'h400000000) || ((remainder == 35'h400000000) && quotient[0]);
      rounded_magnitude = {7'b0, quotient} + increment;
      overflow = (!sign_value && {1'b0, rounded_magnitude} > 33'd2147483647) ||
          (sign_value && {1'b0, rounded_magnitude} > 33'd2147483648);
      if (overflow) result = sign_value ? 32'sh8000_0000 : 32'sh7fff_ffff;
      else if (sign_value) result = -$signed(rounded_magnitude);
      else result = $signed(rounded_magnitude);
      ...
      unique case (state)
        ST_WAIT_INPUT: begin
          if (cfo_valid && cfo_ready) begin
            held_range_error <= cfo_hz < -MAX_ABS_CFO_HZ || cfo_hz > MAX_ABS_CFO_HZ;
