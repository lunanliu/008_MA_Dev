...
module sfo_initial_raw_reader_4lane #(
    ...
    input  logic        [                  31:0] context_frame,
    input  logic        [                  31:0] context_generation,
    ...
    input  logic                                 s_end,
    output logic                                 bank_req_valid,
    input  logic                                 bank_req_ready,
    output logic        [        ADDR_WIDTH-1:0] bank_req_addr,
    output logic        [                  31:0] bank_req_frame,
    output logic        [                  31:0] bank_req_generation,
    input  logic                                 bank_rsp_valid,
    input  logic        [        ADDR_WIDTH-1:0] bank_rsp_addr,
    input  logic        [                  31:0] bank_rsp_frame,
    input  logic        [                  31:0] bank_rsp_generation,
    ...
    output logic                                 context_error_sticky,
  ...
  logic
      fifo_full, fifo_empty, fifo_wr_busy, fifo_rd_busy, fifo_overflow, fifo_underflow, fifo_write;
  logic rsp_match;

  initial begin
    if (OUTPUT_DEPTH < 16 || (OUTPUT_DEPTH & (OUTPUT_DEPTH - 1)) != 0 ||
        (1 << ADDR_WIDTH) < DEPTH_BEATS)
      $fatal(1, "T06_RAW_READER_PARAMETERS");
    if ($bits(metadata_t) != META_WIDTH) $fatal(1, "T06_RAW_READER_METADATA_WIDTH");
  end
  always_comb begin
    context_code = {
      context_valid, context_frame, context_generation, context_origin, context_sample_count
    };
    ...
    bad_range = relative_sample < 0 ||
        relative_sample + 33'sd3 >= $signed({1'b0, context_sample_count}) ||
    ...
    first_addr = relative_sample[ADDR_WIDTH+1:2];
    offset = relative_sample[1:0];
    input_meta = '{
        frame_id: s_frame,
        ...
        beat_index: s_beat,
        phase_codes: s_phase,
        transaction_end: s_end,
        tag: s_tag
    };
    healthy = !rst && !range_error_sticky && !context_error_sticky && !protocol_error_sticky &&
        !busy_context_fault;
    has_credit = !fifo_wr_busy && !fifo_rd_busy && pending_count < OUTPUT_DEPTH;
    need_base = offset != 0 &&
        !(cache_issued_valid && !context_changed && first_addr == last_issued_addr);
    s_ready = healthy && has_credit && !second_pending &&
        ((bad_range || bad_context || bad_fields) || bank_req_ready);
    bank_req_valid = healthy &&
        (second_pending || (s_valid && has_credit && !bad_range && !bad_context && !bad_fields));
    bank_req_addr = second_pending ?
        saved_second_addr : (first_addr + ((offset != 0 && !need_base) ? 1'b1 : 1'b0));
    ...
    rsp_match = bank_rsp_valid == pipe_valid[1] &&
        (!bank_rsp_valid || (bank_rsp_addr == pipe_addr[1] && bank_rsp_frame == pipe_meta[
                             1].frame_id && bank_rsp_generation == pipe_meta[1].generation));
    merged_words = {bank_rsp_data, previous_word};
    fifo_din = {
      pipe_meta[1], pipe_join[1] ? (128'(merged_words >> (32 * pipe_offset[1]))) : bank_rsp_data
    };
    ...
    m_frame = output_meta.frame_id;
    m_generation = output_meta.generation;
    m_abs = output_meta.abs_sample;
    m_symbol = output_meta.symbol_id;
    m_beat = output_meta.beat_index;
    m_phase = output_meta.phase_codes;
    m_end = output_meta.transaction_end;
    m_tag = output_meta.tag;
    for (int lane = 0; lane < 4; lane++) begin
      m_i[16*lane+:16] = output_data[32*lane+:16];
      m_q[16*lane+:16] = output_data[32*lane+16+:16];
    end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      pending_count <= '0;
      range_error_sticky <= 0;
      context_error_sticky <= 0;
      protocol_error_sticky <= 0;
      second_pending <= 0;
      cache_issued_valid <= 0;
      active_context_valid <= 0;
      active_context <= '0;
      last_issued_addr <= '0;
      saved_second_addr <= '0;
      saved_meta <= '0;
      ...
      pipe_valid[0]  <= bank_fire;
      pipe_valid[1]  <= pipe_valid[0];
      pipe_emit[0]   <= issue_emit;
      pipe_emit[1]   <= pipe_emit[0];
      pipe_join[0]   <= issue_join;
      pipe_join[1]   <= pipe_join[0];
      pipe_meta[0]   <= issue_meta;
      pipe_meta[1]   <= pipe_meta[0];
      pipe_addr[0]   <= bank_req_addr;
      pipe_addr[1]   <= pipe_addr[0];
      pipe_offset[0] <= issue_offset;
      pipe_offset[1] <= pipe_offset[0];
      ...
      if (!rsp_match || fifo_overflow || fifo_underflow || (fifo_write && fifo_full) ||
          (output_fire && pending_count == 0))
        protocol_error_sticky <= 1;
      if (context_changed && pending_count == 0 && !second_pending && !(|pipe_valid))
        cache_issued_valid <= 0;
      if (input_fire) begin
      ...
      case ({
        input_fire && !bad_range && !bad_context && !bad_fields, output_fire
      })
      ...
      endcase
      if (bank_fire) begin
        active_context <= context_code;
        active_context_valid <= 1;
        cache_issued_valid <= 1;
        last_issued_addr <= bank_req_addr;
        if (second_pending) second_pending <= 0;
        else if (need_base) begin
          second_pending <= 1;
          saved_second_addr <= first_addr + 1'b1;
          saved_meta <= input_meta;
          saved_offset <= offset;
        end
      end
    end
  ...
  xpm_fifo_sync #(
      ...
      .ECC_MODE("no_ecc"),
      .SIM_ASSERT_CHK(0),
      .FIFO_WRITE_DEPTH(OUTPUT_DEPTH),
      .WRITE_DATA_WIDTH(FIFO_WIDTH),
      .READ_DATA_WIDTH(FIFO_WIDTH),
      .WR_DATA_COUNT_WIDTH($clog2(OUTPUT_DEPTH) + 1),
      .RD_DATA_COUNT_WIDTH($clog2(OUTPUT_DEPTH) + 1),
      .PROG_FULL_THRESH(OUTPUT_DEPTH - 8),
      .PROG_EMPTY_THRESH(8),
      .READ_MODE("fwft"),
      .FIFO_READ_LATENCY(0),
      .DOUT_RESET_VALUE("0"),
