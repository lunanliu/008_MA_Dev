...
module sfo_initial_ps3_ps10_job_sequencer (
    input  logic                clk,
    input  logic                rst_n,
  ...
  always_comb begin
    ...
    proposed_range_error = proposed_first < -33'sd2147483648 || proposed_last > 33'sd2147483647;
    start_ready = rst_n && !busy;
    read_req_valid = rst_n && busy;
    read_req_transaction_end = read_req_beat_index == 9'd511;
    for (int lane = 0; lane < 4; lane++) read_req_phase_codes[32*lane+:32] = phase[lane];
  end
